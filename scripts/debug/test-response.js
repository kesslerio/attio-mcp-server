...
async function testResponse() {
  try {
    ...
    if (response.content && response.content[0]) {
      ...
      console.log('content[0].text:', response.content[0].text?.substring(0, 200) + '...');
      console.log('content[0].text includes "not found":', response.content[0].text?.includes('not found'));
      console.log('content[0].text includes "error":', response.content[0].text?.toLowerCase().includes('error'));
    }
