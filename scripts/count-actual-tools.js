...
async function countActualTools() {
  ...
  try {
    ...
    try {
      ...
      const registryPath = path.resolve(__dirname, '../src/handlers/tools/registry.ts');
      registry = await import(registryPath);
    }
    ...
    Object.entries(registry.TOOL_DEFINITIONS).forEach(([category, definitions]) => {
      let categoryCount = 0;
    ...
    toolList
      ...
      .forEach((tool, index) => {
        console.log(`   ${(index + 1).toString().padStart(2, '0')}. ${tool.name} (${tool.category})`);
      });
