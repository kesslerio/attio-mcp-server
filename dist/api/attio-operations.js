 ...
 */
export async function listObjects(objectType, limit = 20, retryConfig) {
    const api = getAttioClient();
    const path = `/objects/${objectType}/records/query`;
    return callWithRetry(async () => {
        try {
    ...
    return callWithRetry(async () => {
        ...
        try {
            ...
            try {
                ...
                const fallbackResponse = await api.post(fallbackPath, requestBody);
                ...
                try {
                    ...
                    if (filters && filters.filters && filters.filters.length > 0) {
                        throw new Error('GET endpoint cannot be used with filters. This is a limitation of the Attio API. ' +
                    ...
                    const lastResponse = await api.get(lastPath);
...
//# sourceMappingURL=attio-operations.js.map