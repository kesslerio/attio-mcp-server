...
export async function listPrompts(req, res) {
    try {
        ...
        const errorResult = createErrorResult(errorObj, error.message || 'Unknown error', 500);
    ...
    try {
        ...
        if (!template) {
            try {
                ...
                const errorResult = createErrorResult(errorObj, `Template compilation error for prompt ${promptId}: ${compileError.message || 'Unknown error'}`, 500);
                res.status(Number(errorResult.error.code)).json(errorResult);
                return;
            }
        }
        ...
        try {
            ...
            const errorResult = createErrorResult(errorObj, `Template rendering error for prompt ${promptId}: ${renderError.message || 'Unknown error'}`, 500);
            res.status(Number(errorResult.error.code)).json(errorResult);
            return;
        }
        res.json({
