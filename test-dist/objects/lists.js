    ...
    try {
        ...
        try {
            ...
            if (status === 404) {
                ...
                const { ResourceNotFoundError } = await import('../errors/api-errors.js');
                throw new ResourceNotFoundError('list', String(listId), '/lists', 'GET');
            }
            if (status === 422) {
                const { InvalidRequestError } = await import('../errors/api-errors.js');
                throw new InvalidRequestError('Invalid parameter(s) for list operation', '/lists', 'GET');
            }
            throw apiError;
        }
    }
...
async function tryMultipleListEntryEndpoints(listId, limit, offset, filters) {
    ...
    for (const endpoint of endpoints) {
        try {
            if (process.env.NODE_ENV === 'development') {
                console.error(`Trying ${endpoint.method.toUpperCase()} ${endpoint.path}`, endpoint.data ? JSON.stringify(endpoint.data) : '');
            }
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                console.error(`[tryMultipleListEntryEndpoints] [${messageType}] Found ${entries.length} entries via ${endpoint.method.toUpperCase()} ${endpoint.path}`, {
                    listId,
                    limit,
                    offset,
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[tryMultipleListEntryEndpoints] [ERROR] Failed ${endpoint.method.toUpperCase()} ${endpoint.path}:`, errorMessage, {
                    listId,
                    limit,
                    offset,
                    hasFilters: filters && filters.filters ? filters.filters.length > 0 : false,
            ...
            if (status === 404) {
                const { ResourceNotFoundError } = await import('../errors/api-errors.js');
                throw new ResourceNotFoundError('list', String(listId), `/lists/${listId}/entries`, endpoint.method.toUpperCase());
            }
...
export async function getListEntries(listId, limit = 20, offset = 0, filters) {
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        if (process.env.NODE_ENV === 'development') {
            console.error(`[getListEntries] Generic list entries failed: ${errorMessage}`, {
...
export async function addRecordToList(listId, recordId, objectType, initialValues) {
    ...
    if (!Object.values(ResourceType).includes(objectType)) {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`Generic addRecordToList failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            console.error(`Falling back to direct implementation for list ${listId} and record ${recordId}`);
        }
        // Fallback implementation
        const api = getAttioClient();
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[addRecordToList:fallback] Request to ${path} with payload:`, JSON.stringify(payload));
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[addRecordToList:fallback] Success response:`, JSON.stringify(response.data || {}));
            }
            return response.data.data || response.data;
        }
        catch (error) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[addRecordToList] Error adding record ${recordId} to list ${listId}:`, error instanceof Error ? error.message : 'Unknown error');
            ...
            if (hasErrorResponse(error) && error.response?.status === 400) {
                ...
                const errorDetails = validationErrors
                    ...
                    .join('; ');
                throw new Error(`Validation error adding record to list: ${errorDetails ||
...
export async function updateListEntry(listId, entryId, attributes) {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`Generic updateListEntry failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            console.error(`Falling back to direct implementation for list ${listId}, entry ${entryId}`);
        }
        // Fallback implementation
        const api = getAttioClient();
        const path = `/lists/${listId}/entries/${entryId}`;
        if (process.env.NODE_ENV === 'development') {
            console.error(`[updateListEntry:fallback] Request to ${path} with attributes:`, JSON.stringify(attributes));
        }
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[updateListEntry:fallback] Success response:`, JSON.stringify(response.data || {}));
        }
        return response.data.data || response.data;
    }
}
...
export async function removeRecordFromList(listId, entryId) {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`Generic removeRecordFromList failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
        // Fallback implementation
        const api = getAttioClient();
        const path = `/lists/${listId}/entries/${entryId}`;
    ...
    try {
        ...
        for (const objType of objectTypes) {
            try {
                ...
                const response = await api.get(`/objects/${objType}/records/${recordId}/entries`);
                const entries = response?.data?.data || [];
                ...
                if (process.env.NODE_ENV === 'development') {
                    console.warn(`Error checking ${objType} entries for record ${recordId}:`, error);
                }
            }
        }
        return memberships;
        ...
        if (process.env.NODE_ENV === 'development') {
            console.warn(`Error in getRecordListMemberships for record ${recordId}:`, error);
        }
        return [];
    }
}
 ...
 * );
 */
export async function filterListEntries(listId, attributeSlug, condition, value, limit = 20, offset = 0) {
    // Input validation
    if (!listId || typeof listId !== 'string') {
        throw new Error('Invalid list ID: Must be a non-empty string');
    }
        ...
        filters: [
            {
                ...
                condition,
                value: value, // Cast to FilterValue type
            },
        ],
        matchAny: false,
    };
 ...
 * );
 */
export async function advancedFilterListEntries(listId, filters, limit = 20, offset = 0) {
    // Input validation
    if (!listId || typeof listId !== 'string') {
        throw new Error('Invalid list ID: Must be a non-empty string');
    }
...
export async function filterListEntriesByParent(listId, parentObjectType, parentAttributeSlug, condition, value, limit = 20, offset = 0) {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[filterListEntriesByParent] Filtering list ${listId} with path-based filter:`);
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[filterListEntriesByParent] Found ${entries.length} matching entries`);
        }
        return entries;
    }
    catch (error) {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[filterListEntriesByParent] Error filtering list entries: ${error instanceof Error ? error.message : 'Unknown error'}`);
            if (hasErrorResponse(error)) {
                console.error('Status:', error.response?.status);
                console.error('Response data:', JSON.stringify(error.response?.data || {}));
            }
        ...
        if (hasErrorResponse(error) && error.response?.status === 400) {
            throw new Error(`Invalid filter parameters: ${error instanceof Error ? error.message : 'Bad request'}`);
        }
 ...
 */
export async function createList(attributes) {
    // Input validation
    ...
    if (!attributes.parent_object) {
        throw new Error('Parent object type is required (e.g., "companies", "people")');
    }
    const api = getAttioClient();
    const path = '/lists';
    try {
        if (process.env.NODE_ENV === 'development') {
            console.error(`[createList] Creating list with attributes:`, JSON.stringify(attributes));
        }
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[createList] Error:`, error instanceof Error ? error.message : 'Unknown error');
            if (hasErrorResponse(error)) {
                console.error('Status:', error.response?.status);
                console.error('Response data:', JSON.stringify(error.response?.data || {}));
            }
        ...
        // Add context to error message
        if (hasErrorResponse(error) && error.response?.status === 400) {
            throw new Error(`Invalid list attributes: ${error instanceof Error ? error.message : 'Bad request'}`);
        }
 ...
 */
export async function updateList(listId, attributes) {
    // Input validation
    if (!listId || typeof listId !== 'string') {
        throw new Error('Invalid list ID: Must be a non-empty string');
    }
    ...
    try {
        if (process.env.NODE_ENV === 'development') {
            console.error(`[updateList] Updating list ${listId} with attributes:`, JSON.stringify(attributes));
        }
        const response = await api.patch(path, {
            data: attributes,
        });
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[updateList] Error:`, error instanceof Error ? error.message : 'Unknown error');
            if (hasErrorResponse(error)) {
                console.error('Status:', error.response?.status);
                console.error('Response data:', JSON.stringify(error.response?.data || {}));
            }
...
export async function deleteList(listId) {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[deleteList] Error:`, error instanceof Error ? error.message : 'Unknown error');
            if (hasErrorResponse(error)) {
                console.error('Status:', error.response?.status);
                console.error('Response data:', JSON.stringify(error.response?.data || {}));
            }
 ...
 */
export async function searchLists(query, limit = 20, offset = 0) {
...
export async function getListAttributes() {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[getListAttributes] Error:`, error instanceof Error ? error.message : 'Unknown error');
        }
