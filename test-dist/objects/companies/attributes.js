...
export async function getCompanyFields(companyIdOrUri, fields) {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[getCompanyFields] Fetching fields for company ${companyId}:`, fields);
        }
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[getCompanyFields] Filtered to ${Object.keys(filteredValues).length} fields`);
        }
        return result;
    }
    catch (error) {
...
export async function discoverCompanyAttributes() {
    ...
    if (process.env.NODE_ENV === 'development') {
        console.error('[discoverCompanyAttributes] Starting attribute discovery...');
    }
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[discoverCompanyAttributes] Using sample company ID: ${sampleCompanyId}`);
        }
        const sampleCompany = await getCompanyDetails(sampleCompanyId);
        const values = sampleCompany.values || {};
        if (process.env.NODE_ENV === 'development') {
            console.error(`[discoverCompanyAttributes] Retrieved ${Object.keys(values).length} fields from sample company`);
        }
        const standardFields = new Set([
            'name',
            'website',
        ...
        if (process.env.NODE_ENV === 'development') {
            console.error(`[discoverCompanyAttributes] Discovery complete. Found ${standard.length} standard fields and ${custom.length} custom fields.`);
        }
        return result;
    }
    catch (error) {
