    ...
    try {
        ...
        if (isUri) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[getCompanyNotes] Extracted company ID ${companyId} from URI ${companyIdOrUri}`);
            }
        }
        else {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[getCompanyNotes] Using direct company ID: ${companyId}`);
            }
        }
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[getCompanyNotes] Unified operation failed: ${error.message || 'Unknown error'}`, error);
            }
...
export async function createCompanyNote(companyIdOrUri, title, content) {
    ...
    try {
        ...
        if (isUri) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[createCompanyNote] Extracted company ID ${companyId} from URI ${companyIdOrUri}`);
            }
        }
        else {
            // Direct ID was provided
            companyId = companyIdOrUri;
            if (process.env.NODE_ENV === 'development') {
                console.error(`[createCompanyNote] Using direct company ID: ${companyId}`);
            }
        }
        // Validate that we have a non-empty ID
        if (!companyId || companyId.trim() === '') {
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[createCompanyNote] Unified operation failed: ${error.message || 'Unknown error'}`, error);
            }
            // Fallback implementation with better error handling
            try {
                const api = getAttioClient();
