    ...
    try {
        ...
        if (isUri) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[getCompanyDetails] Extracted company ID ${companyId} from URI ${companyIdOrUri}`);
            }
        }
        else {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[getCompanyDetails] Using direct company ID: ${companyId}`);
            }
        }
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[getCompanyDetails] First attempt failed: ${firstError instanceof Error ? firstError.message : 'Unknown error'}`, {
            ...
            try {
                ...
                if (process.env.NODE_ENV === 'development') {
                    console.error(`[getCompanyDetails] Second attempt failed: ${secondError instanceof Error
                        ? secondError.message
                        : 'Unknown error'}`, {
                ...
                try {
                    ...
                    if (process.env.NODE_ENV === 'development') {
                        console.error(`[getCompanyDetails] Trying alternate path: ${alternatePath}`, {
                    ...
                    const errorDetails = {
                        ...
                        errors: {
                            first: firstError instanceof Error
                                ? firstError.message
                                : 'Unknown error',
                            second: secondError instanceof Error
                                ? secondError.message
                                : 'Unknown error',
                            third: thirdError instanceof Error
                                ? thirdError.message
                                : 'Unknown error',
                        },
                    };
                    ...
                    throw new Error(`Could not retrieve company details for ${companyIdOrUri}: ${thirdError instanceof Error ? thirdError.message : 'Unknown error'}`);
                }
            }
        }
    }
...
export async function createCompany(attributes) {
    ...
    try {
        console.error('[createCompany] DEBUG - Input attributes:', JSON.stringify(attributes, null, 2));
        // Temporarily comment out validation to isolate the issue
        const result = await createObjectWithDynamicFields(ResourceType.COMPANIES, attributes
        // CompanyValidator.validateCreate  // Temporarily disabled
        );
        console.error('[createCompany] DEBUG - Result from createObjectWithDynamicFields:', JSON.stringify(result, null, 2));
        if (process.env.NODE_ENV === 'development') {
            console.error('[createCompany] Result from createObjectWithDynamicFields:', {
                result,
                hasId: !!result?.id,
        ...
        if (!result) {
            throw new CompanyOperationError('create', undefined, 'API returned null/undefined response for company creation');
        }
        if (!result.id || !result.id.record_id) {
            throw new CompanyOperationError('create', undefined, `API returned invalid company record without proper ID structure. Response: ${JSON.stringify(result)}`);
        }
        if (!result.values || typeof result.values !== 'object') {
            throw new CompanyOperationError('create', undefined, `API returned invalid company record without values object. Response: ${JSON.stringify(result)}`);
        }
        return result;
    }
    catch (error) {
...
export async function updateCompanyAttribute(companyId, attributeName, attributeValue) {
    try {
         ...
         */
        if (attributeName === 'main_contact' &&
            typeof attributeValue === 'string') {
            // Use the utility function to handle person reference lookup
            valueToProcess = await findPersonReference(attributeValue, 'update attribute', 'company', companyId);
        }
        ...
        const processedValue = await CompanyValidator.validateAttributeUpdate(companyId, attributeName, valueToProcess // TODO: Replace with proper type once CompanyFieldValue is updated
        );
        return await updateObjectAttributeWithDynamicFields(ResourceType.COMPANIES, companyId, attributeName, processedValue, updateCompany);
    }
    catch (error) {
        if (error instanceof InvalidCompanyDataError ||
