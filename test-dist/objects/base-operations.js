...
function translateAttributeNames(objectType, attributes) {
    ...
    for (const [userKey, value] of Object.entries(attributes)) {
        ...
        if (process.env.NODE_ENV === 'development' && userKey !== apiKey) {
            console.error(`[translateAttributeNames:${objectType}] Mapped "${userKey}" -> "${apiKey}"`);
        }
...
export async function createObjectWithDynamicFields(objectType, attributes, validator) {
    ...
    if (process.env.NODE_ENV === 'development') {
        console.error(`[createObjectWithDynamicFields:${objectType}] Original attributes:`, JSON.stringify(validatedAttributes, null, 2));
        console.error(`[createObjectWithDynamicFields:${objectType}] Mapped attributes:`, JSON.stringify(mappedAttributes, null, 2));
        console.error(`[createObjectWithDynamicFields:${objectType}] Final transformed attributes:`, JSON.stringify(transformedAttributes, null, 2));
    }
    try {
        // Create the object
        const result = await createObjectRecord(objectType, transformedAttributes);
        if (process.env.NODE_ENV === 'development') {
            console.error(`[createObjectWithDynamicFields:${objectType}] Result from createObjectRecord:`, {
                result,
                hasId: !!result?.id,
...
export async function updateObjectWithDynamicFields(objectType, recordId, attributes, validator) {
    ...
    const transformedAttributes = await formatAllAttributes(objectType, mappedAttributes);
    if (process.env.NODE_ENV === 'development') {
        console.error(`[updateObjectWithDynamicFields:${objectType}] Original attributes:`, JSON.stringify(validatedAttributes, null, 2));
        console.error(`[updateObjectWithDynamicFields:${objectType}] Mapped attributes:`, JSON.stringify(mappedAttributes, null, 2));
        console.error(`[updateObjectWithDynamicFields:${objectType}] Final transformed attributes:`, JSON.stringify(transformedAttributes, null, 2));
    }
    // Update the object
