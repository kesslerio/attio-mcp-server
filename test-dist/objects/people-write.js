...
import { PersonOperationError, InvalidPersonDataError, } from './people/errors.js';
    ...
    static async validateCreate(attributes) {
        ...
        if (attributes.company && typeof attributes.company === 'string') {
            ...
            if (results.length === 1) {
                ...
                throw new InvalidPersonDataError(`Company '${companyName}' not found. Provide a valid company ID.`);
            }
            else {
                throw new InvalidPersonDataError(`Multiple companies match '${companyName}'. Please specify the company ID.`);
            }
        }
        return attributes;
    }
