 ...
 */
export async function getResourceAttributes(resourceType) {
 ...
 */
export async function validateSelectField(resourceType, fieldName, value) {
    try {
 ...
 */
export async function validateMultiSelectField(resourceType, fieldName, values) {
    try {
        const attributes = await getResourceAttributes(resourceType);
        const field = attributes.find((attr) => attr.api_slug === fieldName);
 ...
 */
export async function validateReadOnlyFields(resourceType, updateFields) {
    try {
        const attributes = await getResourceAttributes(resourceType);
        const readOnlyFields = attributes
            .filter((attr) => attr.read_only === true)
            .map((attr) => attr.api_slug);
        const attemptedReadOnlyUpdates = Object.keys(updateFields).filter((fieldName) => readOnlyFields.includes(fieldName));
...
function calculateSimilarity(a, b) {
    ...
    for (let j = 1; j <= b.length; j++) {
        ...
        for (let i = 1; i <= a.length; i++) {
            ...
            currRow[i] = Math.min(currRow[i - 1] + 1, // deletion
 ...
 */
export async function suggestFieldName(resourceType, invalidFieldName) {
    try {
        const attributes = await getResourceAttributes(resourceType);
 ...
 */
export async function validateFieldExistence(resourceType, fieldNames) {
    try {
        const attributes = await getResourceAttributes(resourceType);
 ...
 */
export async function validateRecordFields(resourceType, fields, isUpdate = false) {
