...
import { generateFieldSuggestionMessage, generateEnumSuggestionMessage, generateReadOnlyFieldMessage, generateResourceTypeSuggestionMessage, VALID_RESOURCE_TYPES, } from './field-suggestions.js';
/**
 * Get helpful examples based on error context
 */
export function getErrorExamples(errorType, context) {
    const examples = [];
    switch (errorType) {
        case ErrorType.PARAMETER_ERROR:
            ...
            else if (context.paramName === 'updates' ||
 ...
 */
export function enhanceErrorMessage(originalMessage, errorType, context) {
    ...
    if (errorType === ErrorType.VALIDATION_ERROR ||
        ...
        if (context.fieldName &&
            ...
            if (!context.validFields.includes(context.fieldName)) {
                enhancedMessage = generateFieldSuggestionMessage(context.fieldName, context.validFields, context.resourceType);
            }
            else if (context.isReadOnly) {
                enhancedMessage = generateReadOnlyFieldMessage(context.fieldName, context.operation);
            }
        }
        ...
        if (context.fieldName &&
            ...
            if (context.actualValue !== undefined) {
                ...
                if (!context.validValues.includes(valueStr)) {
                    enhancedMessage = generateEnumSuggestionMessage(context.actualValue, context.validValues, context.fieldName);
                }
            }
        }
        ...
        if (context.resourceType &&
            ...
            enhancedMessage = generateResourceTypeSuggestionMessage(context.resourceType, VALID_RESOURCE_TYPES);
        }
    }
