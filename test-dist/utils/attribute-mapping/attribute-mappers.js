    ...
    try {
        ...
        if (specialCaseResult) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Special case match: "${attributeName}" -> "${specialCaseResult}"`);
            }
        ...
        if (objectType) {
            ...
            if (objectSpecificCache) {
                ...
                if (result) {
                    if (process.env.NODE_ENV === 'development') {
                        console.error(`[attribute-mappers] Object-specific case-insensitive match for ${objectType}: "${attributeName}" -> "${result}"`);
                    }
                    return result;
                }
            }
        ...
        if (result) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Custom case-insensitive match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
        ...
        if (result) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Common case-insensitive match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
        ...
        if (result) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Legacy case-insensitive match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
        ...
        if (objectType) {
            ...
            if (normalizedObjectSpecificCache) {
                ...
                if (result) {
                    if (process.env.NODE_ENV === 'development') {
                        console.error(`[attribute-mappers] Object-specific normalized match for ${objectType}: "${attributeName}" -> "${result}"`);
                    }
                    return result;
                }
            }
        ...
        if (result) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Custom normalized match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
        ...
        if (result) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Common normalized match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
        ...
        if (result) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Legacy normalized match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
        ...
        if (result) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Aggressive normalized match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
        ...
        if (result && result !== attributeName) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[attribute-mappers] Snake case conversion match: "${attributeName}" -> "${result}"`);
            }
            return result;
        }
    }
        ...
        const specialCaseResult = handleSpecialCases(attributeName);
        if (specialCaseResult) {
            console.error(`[attribute-mappers] Special case match after error: "${attributeName}" -> "${specialCaseResult}"`);
            return specialCaseResult;
        }
    }
