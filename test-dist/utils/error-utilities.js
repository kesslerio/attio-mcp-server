 ...
 * }
 */
export function handleAxiosError(error, operation) {
    ...
    if (error.response) {
        ...
        return new Error(`Failed to ${operation}: ${status} ${statusText}\n${errorDetails}`);
    }
    if (error.request) {
 ...
 * }
 */
export function createStandardErrorResult(error, operation, resource, additionalDetails) {
 ...
 * }
 */
export function logAndReturn(error, context, details) {
    const message = getErrorMessage(error);
 ...
 * }
 */
export function ensureError(error, fallbackMessage = 'An unknown error occurred') {
    if (error instanceof Error) {
        return error;
    }
 ...
 * );
 */
export async function safeAsync(operation, errorHandler) {
    try {
