...
export function transformFiltersToApiFormat(filters, validateConditions = true, isListEntryContext = false) {
    ...
    try {
        ...
        if (!validatedFilters.filters || validatedFilters.filters.length === 0) {
            if (process.env.NODE_ENV === 'development') {
                console.error('[transformFiltersToApiFormat] Empty or undefined filters array provided, returning empty result');
            }
            return {};
        }
    }
        ...
        if (error instanceof FilterValidationError) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error('[transformFiltersToApiFormat] Validation failed, returning empty result:', error);
            }
            return {};
        }
    ...
    if (process.env.NODE_ENV === 'development') {
        console.error(`[transformFiltersToApiFormat] Using ${useOrLogic ? 'OR' : 'AND'} logic for filters`);
        console.error(`[transformFiltersToApiFormat] Processing ${validatedFilters.filters.length} filter conditions`);
    }
...
function createOrFilterStructure(filters, validateConditions, isListEntryContext = false) {
    ...
    filters.forEach((filter, index) => {
        ...
        if (isListEntryContext && isListSpecificAttribute(slug)) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[OR Logic] Using list-specific attribute format for field ${slug}`);
            }
        ...
        else if (FIELD_SPECIAL_HANDLING[slug] &&
            FIELD_SPECIAL_HANDLING[slug].useShorthandFormat) {
            // For special fields that need shorthand format
            if (process.env.NODE_ENV === 'development') {
                console.error(`[OR Logic] Using shorthand filter format for field ${slug}`);
            }
