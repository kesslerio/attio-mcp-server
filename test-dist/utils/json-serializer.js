 ...
 */
export function safeJsonStringify(obj, options = {}) {
    ...
    try {
        ...
        if (duration > 100) {
            console.error(`[safeJsonStringify] Slow serialization detected: ${duration.toFixed(2)}ms for ${typeof obj} (${result.length} chars)`);
        }
        return result;
    }
    catch (error) {
 ...
 */
export function hasCircularReferences(obj, maxDepth = 10) {
 ...
 */
export function createSafeCopy(obj, options = {}) {
    try {
        ...
        console.error('[createSafeCopy] Failed to create safe copy:', error instanceof Error ? error.message : String(error));
 ...
 */
export function sanitizeMcpResponse(response) {
