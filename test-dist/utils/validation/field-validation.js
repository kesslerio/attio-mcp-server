 ...
 */
const RESOURCE_SPECIFIC_FIELDS = {
    [ResourceType.PEOPLE]: [
        ...COMMON_SAFE_FIELDS,
        'first_name',
        'last_name',
 ...
 */
export function sanitizeFieldName(fieldName, options = {}) {
    const { allowUnderscores = true, allowNumbers = true, maxLength = 50, toLowerCase = false, } = options;
 ...
 */
export function validateFieldName(fieldName, resourceType, options = {}) {
    const warnings = [];
    ...
    if (sanitized !== fieldName.trim()) {
        const displayFieldName = fieldName.length > 100 ? fieldName.substring(0, 100) + '...' : fieldName;
        warnings.push(`Field name was sanitized from "${displayFieldName}" to "${sanitized}"`);
    }
    // Validate against known fields for this resource type
    const allowedFields = RESOURCE_SPECIFIC_FIELDS[resourceType] || COMMON_SAFE_FIELDS;
    if (!allowedFields.includes(sanitized)) {
        ...
        if (similarField) {
            warnings.push(`Did you mean "${similarField}" instead of "${sanitized}"?`);
        }
        else {
            warnings.push(`Field "${sanitized}" is not in the known safe fields list for ${resourceType}`);
        }
    }
    return {
        valid: true,
 ...
 */
export function validateFieldNames(fieldNames, resourceType, options = {}) {
 ...
 */
function findSimilarField(target, candidates) {
    let bestMatch = null;
 ...
 * Enhanced for comprehensive security testing
 */
export function secureValidateFields(fieldNames, // Allow any type for comprehensive input validation
resourceType, operation = 'field filtering') {
    // Enhanced input type validation
    if (fieldNames === null || fieldNames === undefined) {
        throw new UniversalValidationError(`Invalid field names for ${operation}: field names cannot be null or undefined`, ErrorType.USER_ERROR, {
            field: 'fields',
            suggestion: 'Provide an array of field names',
        });
    }
    if (!Array.isArray(fieldNames)) {
        throw new UniversalValidationError(`Invalid field names for ${operation}: must be an array, received ${typeof fieldNames}`, ErrorType.USER_ERROR, {
            field: 'fields',
            suggestion: 'Provide an array of field names',
        });
    }
    ...
    if (fieldNames.length > 50) {
        throw new UniversalValidationError(`Too many fields specified for ${operation}: ${fieldNames.length} (maximum: 50)`, ErrorType.USER_ERROR, {
            field: 'fields',
    ...
    for (let i = 0; i < fieldNames.length; i++) {
        ...
        if (typeof field !== 'string') {
            throw new UniversalValidationError(`Invalid field names for ${operation}: all field names must be strings, found ${typeof field} at index ${i}`, ErrorType.USER_ERROR, {
                field: 'fields',
    ...
    if (!validation.valid) {
        throw new UniversalValidationError(`Invalid field names for ${operation}: ${validation.errors.join(', ')}. Valid fields include: ${RESOURCE_SPECIFIC_FIELDS[resourceType]?.slice(0, 10).join(', ')}`, ErrorType.USER_ERROR, {
            field: 'fields',
    ...
    if (validation.warnings.length > 0) {
        console.warn(`Field validation warnings for ${operation}:`, validation.warnings);
    }
...
export function validateCategoryName(categoryName) {
    ...
    if (!VALID_CATEGORIES.includes(sanitized)) {
        ...
        if (similarCategory) {
            warnings.push(`Did you mean "${similarCategory}" instead of "${sanitized}"?`);
        }
        else {
            warnings.push(`Category "${sanitized}" is not in the known categories list`);
        }
    }
    return {
        valid: true,
 ...
 */
export function validateCategoryNames(categoryNames) {
 ...
 * Enhanced for comprehensive security testing
 */
export function secureValidateCategories(categoryNames, // Allow any type for comprehensive input validation
operation = 'category filtering') {
    // Enhanced input type validation
    if (categoryNames === null || categoryNames === undefined) {
        throw new UniversalValidationError(`Invalid category names for ${operation}: category names cannot be null or undefined`, ErrorType.USER_ERROR, {
            field: 'categories',
            suggestion: 'Provide an array of category names',
        });
    }
    if (!Array.isArray(categoryNames)) {
        throw new UniversalValidationError(`Invalid category names for ${operation}: must be an array, received ${typeof categoryNames}`, ErrorType.USER_ERROR, {
            field: 'categories',
            suggestion: 'Provide an array of category names',
        });
    }
    ...
    if (categoryNames.length > 10) {
        throw new UniversalValidationError(`Too many categories specified for ${operation}: ${categoryNames.length} (maximum: 10)`, ErrorType.USER_ERROR, {
            field: 'categories',
    ...
    for (let i = 0; i < categoryNames.length; i++) {
        ...
        if (typeof category !== 'string') {
            throw new UniversalValidationError(`Invalid category names for ${operation}: all category names must be strings, found ${typeof category} at index ${i}`, ErrorType.USER_ERROR, {
                field: 'categories',
    ...
    if (!validation.valid) {
        throw new UniversalValidationError(`Invalid category names for ${operation}: ${validation.errors.join(', ')}. Valid categories: ${VALID_CATEGORIES.join(', ')}`, ErrorType.USER_ERROR, {
            field: 'categories',
    ...
    // Log warnings but don't fail
    if (validation.warnings.length > 0) {
        console.warn(`Category validation warnings for ${operation}:`, validation.warnings);
    }
