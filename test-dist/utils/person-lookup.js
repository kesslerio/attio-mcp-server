 ...
 */
export async function findPersonReference(value, operationContext, recordIdType, recordId) {
    ...
    try {
        ...
        if (people.length === 0) {
            throw new CompanyOperationError(operationContext, recordId, `Person named "${value}" not found. Please provide an exact name or a valid Person Record ID (e.g., person_xxxxxxxxxxxx).`);
        }
        if (people.length > 1) {
            const names = people
                .map((p) => p.values.name?.[0]?.value || 'Unknown Name')
                .join(', ');
            throw new CompanyOperationError(operationContext, recordId, `Multiple people found for "${value}": [${names}]. Please provide a more specific name or the Person Record ID.`);
        }
        const person = people[0];
        const personRecordId = person.id?.record_id;
        if (!personRecordId) {
            throw new CompanyOperationError(operationContext, recordId, `Could not retrieve Record ID for person "${value}".`);
        }
        ...
        // Wrap other errors
        throw new CompanyOperationError(operationContext, recordId, `Failed to search for person "${value}": ${searchError instanceof Error ? searchError.message : String(searchError)}`);
    }
}
/**
 ...
 */
export function clearPersonReference() {
    return [];
}
