...
(function (SensitiveInfoType) {
    SensitiveInfoType["FILE_PATH"] = "file_path";
    SensitiveInfoType["API_KEY"] = "api_key";
    SensitiveInfoType["INTERNAL_ID"] = "internal_id";
    SensitiveInfoType["STACK_TRACE"] = "stack_trace";
    SensitiveInfoType["DATABASE_SCHEMA"] = "database_schema";
    SensitiveInfoType["SYSTEM_INFO"] = "system_info";
    SensitiveInfoType["URL_WITH_PARAMS"] = "url_with_params";
    SensitiveInfoType["EMAIL_ADDRESS"] = "email_address";
    SensitiveInfoType["IP_ADDRESS"] = "ip_address";
...
function classifyError(message) {
    ...
    if (lowerMessage.includes('cannot find attribute') ||
 ...
 */
function extractSafeContext(message) {
...
export function sanitizeErrorMessage(error, options = {}) {
    const { includeContext = true, logOriginal = true, module = 'error-sanitizer', operation = 'sanitize', safeMetadata = {}, } = options;
    ...
    if (logOriginal && process.env.NODE_ENV !== 'production') {
        logError(module, `Original error (internal only): ${originalMessage}`, { name: errorName, stack: stackTrace, ...safeMetadata }, undefined, operation, OperationType.SYSTEM);
    }
 ...
 */
export function createSanitizedError(error, statusCode, options = {}) {
    const sanitizedMessage = sanitizeErrorMessage(error, options);
    const errorType = classifyError(error instanceof Error ? error.message : String(error));
    return {
 ...
 */
function inferStatusCode(errorType) {
    switch (errorType) {
        case 'authentication':
            return 401;
        case 'authorization':
 ...
 */
export function withErrorSanitization(fn, options = {}) {
    return (async (...args) => {
        try {
...
export function getErrorSummary(error) {
    const errorType = classifyError(error instanceof Error ? error.message : String(error));
    const safeContext = extractSafeContext(error instanceof Error ? error.message : String(error));
