 ...
 */
import { BATCH_SIZE_LIMITS, PAYLOAD_SIZE_LIMITS, LIMIT_ERROR_MESSAGES, getBatchSizeLimit, } from '../config/security-limits.js';
 ...
 */
export function validateBatchSize(items, operationType, resourceType) {
    ...
    if (items.length > maxSize) {
        return {
            isValid: false,
            error: LIMIT_ERROR_MESSAGES.BATCH_SIZE_EXCEEDED(items.length, maxSize, `${operationType} ${resourceType || ''}`.trim()),
            errorType: ErrorType.VALIDATION_ERROR,
            details: {
 ...
 * @returns Validation result
 */
export function validatePayloadSize(payload, checkSingleRecords = true) {
    ...
    if (totalSize > PAYLOAD_SIZE_LIMITS.BATCH_TOTAL) {
        return {
            isValid: false,
            error: LIMIT_ERROR_MESSAGES.PAYLOAD_SIZE_EXCEEDED(totalSize, PAYLOAD_SIZE_LIMITS.BATCH_TOTAL),
            errorType: ErrorType.VALIDATION_ERROR,
            details: {
            ...
            if (recordSize > PAYLOAD_SIZE_LIMITS.SINGLE_RECORD) {
                return {
                    ...
                    error: `Record at index ${i}: ` +
                        LIMIT_ERROR_MESSAGES.SINGLE_RECORD_SIZE_EXCEEDED(recordSize, PAYLOAD_SIZE_LIMITS.SINGLE_RECORD),
                    errorType: ErrorType.VALIDATION_ERROR,
                    details: {
 ...
 * @returns Validation result
 */
export function validateSearchQuery(query, filters) {
    // Validate query string length
    if (query && query.length > PAYLOAD_SIZE_LIMITS.SEARCH_QUERY) {
        return {
            isValid: false,
            error: LIMIT_ERROR_MESSAGES.SEARCH_QUERY_TOO_LONG(query.length, PAYLOAD_SIZE_LIMITS.SEARCH_QUERY),
            errorType: ErrorType.VALIDATION_ERROR,
        };
    }
    ...
    if (filters) {
        ...
        if (filterSize > PAYLOAD_SIZE_LIMITS.FILTER_OBJECT) {
            return {
                isValid: false,
                error: LIMIT_ERROR_MESSAGES.FILTER_TOO_COMPLEX(filterSize, PAYLOAD_SIZE_LIMITS.FILTER_OBJECT),
                errorType: ErrorType.VALIDATION_ERROR,
            };
        }
    }
 ...
 */
export function splitBatchIntoChunks(items, resourceType) {
 ...
 */
export function createSafeBatchError(validation) {
