...
export function processListEntries(entries) {
    return entries.map((entry) => {
        ...
        if (recordId) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[processListEntries] Found record_id: ${recordId} for entry ${entry.id?.entry_id || 'unknown'}`);
            }
            return {
                ...entry,
                record_id: recordId,
        ...
        if (entry.record?.uri) {
            ...
            if (uriParts.length > 0) {
                ...
                if (process.env.NODE_ENV === 'development') {
                    console.error(`[processListEntries] Extracted record_id ${recordId} from URI: ${entry.record.uri}`);
                }
                return {
                    ...entry,
                    record_id: recordId,
