 ...
 */
import { RelativeDateUnit, DateRangePreset, } from '../types/attio.js';
 ...
 */
export function resolveRelativeDate(relativeDate) {
    ...
    if (relativeDate.direction !== 'past' &&
    ...
    if (typeof relativeDate.value !== 'number' ||
 ...
 */
export function createDateRangeFromPreset(preset) {
    // Validate preset
    ...
    const isValidPreset = Object.values(DateRangePreset).includes(normalizedPreset);
    if (!isValidPreset) {
...
export function resolveDateRange(dateRange) {
    ...
    if (dateRange.preset) {
        try {
            ...
            if (dateRange.start || dateRange.end) {
                console.warn('DateRange contains both preset and start/end specifications. ' +
    ...
    if (result.start && result.end) {
        ...
        if (startDate > endDate) {
            throw new Error(`Invalid date range: start date (${result.start}) ` +
 ...
 * @returns Object with start and end dates as ISO strings
 */
export function createRelativeDateRange(value, unit) {
    const now = new Date();
 ...
 */
export function validateAndCreateDateRange(startDate, endDate) {
    ...
    if (startDate && startDate.trim() !== '') {
        ...
        if (!isValidISODateString(trimmedStart)) {
            throw new Error(`Invalid start date format. Expected ISO 8601 format (YYYY-MM-DDTHH:mm:ss.sssZ), got: "${trimmedStart}"`);
        }
    ...
    if (endDate && endDate.trim() !== '') {
        ...
        if (!isValidISODateString(trimmedEnd)) {
            throw new Error(`Invalid end date format. Expected ISO 8601 format (YYYY-MM-DDTHH:mm:ss.sssZ), got: "${trimmedEnd}"`);
        }
    ...
    if ((result.start && !result.end) || (!result.start && result.end)) {
        console.info(`Creating open-ended date range: ${result.start ? `from ${result.start}` : `until ${result.end}`}`);
    }
    ...
    if (result.start && result.end) {
        ...
        const endDate = new Date(result.end);
        if (startDate > endDate) {
            throw new Error(`Invalid date range: start date (${result.start}) must be before or equal to end date (${result.end})`);
        }
        // Warn if the date range is suspiciously large
        const diffMs = endDate.getTime() - startDate.getTime();
        const diffYears = diffMs / (1000 * 60 * 60 * 24 * 365);
        if (diffYears > 10) {
            console.warn(`Large date range detected: ${diffYears.toFixed(1)} years. This may impact performance.`);
        }
    }
    return result;
}
