...
export async function addRecordToList(listId, recordId, objectType, initialValues, retryConfig) {
    ...
    return callWithRetry(async () => {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[addRecordToList] Error adding record ${recordId} to list ${listId}:`, listError.message || 'Unknown error');
                console.error('Status:', listError.response?.status);
                console.error('Response data:', JSON.stringify(listError.response?.data || {}));
                // Add additional debug information for validation errors
                if (listError.response?.data?.validation_errors) {
                    console.error('Validation errors:', JSON.stringify(listError.response.data.validation_errors));
                }
            }
            ...
            if (listError.response?.status === 400) {
                ...
                const errorDetails = validationErrors
                    ...
                    .join('; ');
                throw new Error(`Validation error adding record to list: ${errorDetails || listError.message}`);
            }
...
export async function updateListEntry(listId, entryId, attributes, retryConfig) {
    ...
    return callWithRetry(async () => {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.error(`[updateListEntry] Error updating entry ${entryId} in list ${listId}:`, updateError.message || 'Unknown error');
            ...
            if (updateError.response?.status === 404) {
                ...
                throw new Error(`Invalid attributes for list entry update: ${updateError.response?.data?.message || 'Bad request'}`);
            }
