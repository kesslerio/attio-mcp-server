 ...
 */
export async function getObjectAttributeMetadata(objectSlug) {
    // Check cache first
 ...
 */
function createTaskAttributeMetadata() {
    const taskFields = new Map();
    // Standard task fields based on Attio Tasks API
    const createTaskField = (slug, type, title, required = false) => ({
        id: {
            workspace_id: 'tasks',
            object_id: 'tasks',
            attribute_id: slug,
    ...
    taskFields.set('record_id', createTaskField('record_id', 'text', 'Record ID'));
 ...
 */
export async function detectFieldType(objectSlug, attributeSlug) {
    ...
    const isMultiple = attrMetadata.is_multiselect ||
        ('allow_multiple_values' in attrMetadata &&
            attrMetadata.allow_multiple_values);
 ...
 */
export async function getAttributeTypeInfo(objectSlug, attributeSlug) {
    const metadata = await getObjectAttributeMetadata(objectSlug);
    const attrMetadata = metadata.get(attributeSlug);
    if (process.env.NODE_ENV === 'development') {
        console.error(`[getAttributeTypeInfo] Looking up ${objectSlug}.${attributeSlug}:`, {
    ...
    if (!attrMetadata) {
        if (process.env.NODE_ENV === 'development') {
            console.error(`[getAttributeTypeInfo] No metadata found for ${objectSlug}.${attributeSlug}, returning default`);
        }
        return {
            fieldType: 'string',
            isArray: false,
 ...
 */
export function clearAttributeCache(objectSlug) {
    if (objectSlug) {
 ...
 */
export async function getAttributeSlugById(objectSlug, attributeId) {
    const metadata = await getObjectAttributeMetadata(objectSlug);
 ...
 */
export async function getFieldValidationRules(objectSlug, attributeSlug) {
    ...
    if (typeInfo.metadata) {
        ...
        if (typeInfo.attioType === 'select') {
            ...
            if (options) {
                rules.enum = options.map((opt) => opt.value);
            }
        }
    }
    return rules;
 ...
 */
export async function formatAttributeValue(objectSlug, attributeSlug, value) {
    const typeInfo = await getAttributeTypeInfo(objectSlug, attributeSlug);
