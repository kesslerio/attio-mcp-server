...
import { validateResourceType, getFieldSuggestions, } from '../handlers/tool-configs/universal/field-mapper.js';
/**
...
export class ErrorService {
     ...
     */
    static createUniversalError(operation, resourceType, originalError) {
        ...
        if (originalError instanceof UniversalValidationError ||
            (originalError &&
                ...
                (originalError.constructor.name === 'EnhancedApiError' ||
                    originalError.name === 'EnhancedApiError'))) {
        ...
        return new UniversalValidationError(message, errorType, {
            suggestion: this.getOperationSuggestion(operation, resourceType, originalError),
            cause: originalError,
        });
    }
    /**
     ...
     */
    static getOperationSuggestion(operation, resourceType, error) {
        ...
        if (lowerErrorMessage.includes('date range') ||
        ...
        if (lowerErrorMessage.includes('unable to parse date') ||
        ...
        if (lowerErrorMessage.includes('filter') &&
        ...
        if (lowerErrorMessage.includes('batch') &&
        ...
        if (lowerErrorMessage.includes('cannot find attribute')) {
            const errorMessageForMatch = error instanceof Error
                ? error.message
            ...
            if (match && match[1]) {
                ...
                if (Object.values(UniversalResourceType).includes(resourceType)) {
                    const suggestion = getFieldSuggestions(resourceType, fieldName);
                    if (suggestion) {
                        return suggestion;
                    }
                }
        ...
        if (lowerErrorMessage.includes('cannot find attribute')) {
            const attrMatch = lowerErrorMessage.match(/cannot find attribute with slug\/id["\s]*([^"]*)/);
     ...
     */
    static getDealSpecificSuggestion(errorMessage) {
        ...
        if (errorMessage.includes('expected_close_date') ||
        ...
        if (errorMessage.includes('probability') ||
