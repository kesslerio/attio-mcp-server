...
export class UniversalRetrievalService {
    ...
    static async getRecordDetails(params) {
        ...
        const perfId = enhancedPerformanceTracker.startOperation('get-record-details', 'get', { resourceType: resource_type, recordId: record_id });
        ...
        try {
            ...
            if (fields && fields.length > 0) {
                ...
                return {
                    ...
                    values: filteredResult.values || result.values,
                };
            }
            return result;
        }
            ...
            if (statusCode === 404 ||
                ...
                ErrorEnhancer.getErrorMessage(enhancedError), 404);
                throw enhancedError;
            }
            // Auto-enhance other errors with context
            const error = apiError instanceof Error ? apiError : new Error(String(apiError));
            const enhancedError = ErrorEnhancer.autoEnhance(error, resource_type, 'get-record-details', record_id);
            enhancedPerformanceTracker.endOperation(perfId, false, 
            // Issue #425: Use safe error message extraction
            ErrorEnhancer.getErrorMessage(enhancedError), statusCode);
            throw enhancedError;
        }
    }
    /**
     ...
     */
    static async retrieveRecordByType(resource_type, record_id) {
     ...
     */
    static async retrieveListRecord(record_id) {
     ...
     */
    static async retrieveTaskRecord(record_id, resource_type) {
        try {
     ...
     */
    static async retrieveNoteRecord(noteId) {
        try {
     ...
     */
    static filterResponseFields(data, requestedFields) {
     ...
     */
    static async recordExists(resource_type, record_id) {
        try {
     ...
     */
    static async getMultipleRecords(resource_type, record_ids, fields) {
        // For now, fetch records individually
        // TODO: Implement batch API calls where supported by Attio
        const results = await Promise.allSettled(record_ids.map((record_id) => this.getRecordDetails({ resource_type, record_id, fields })));
        return results.map((result) => result.status === 'fulfilled' ? result.value : null);
    }
    /**
     * Get record with performance metrics
     */
    static async getRecordWithMetrics(params) {
        const start = performance.now();
        // Check if response is cached
        const isCached = CachingService.isCached404(params.resource_type, params.record_id);
