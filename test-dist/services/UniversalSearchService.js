...
const ensureAdvancedSearchCompanies = async () => {
    try {
        ...
        console.log('Checking advancedSearchCompanies availability:', typeof advancedSearchCompanies);
        if (typeof advancedSearchCompanies !== 'function') {
            console.error('advancedSearchCompanies is not a function:', advancedSearchCompanies);
            return null;
        }
...
const ensureAdvancedSearchPeople = async () => {
    try {
        console.log('Checking advancedSearchPeople availability:', typeof advancedSearchPeople);
        if (typeof advancedSearchPeople !== 'function') {
            console.error('advancedSearchPeople is not a function:', advancedSearchPeople);
            return null;
        }
...
export class UniversalSearchService {
    ...
    static async searchRecords(params) {
        const { resource_type, query, filters, limit, offset, search_type = SearchType.BASIC, fields, match_type = MatchType.PARTIAL, sort = SortType.NAME, } = params;
        // Start performance tracking
        const perfId = enhancedPerformanceTracker.startOperation('search-records', 'search', {
        ...
        try {
            ...
            enhancedPerformanceTracker.endOperation(perfId, false, errorMessage, statusCode);
            throw apiError;
        }
    }
    /**
     ...
     */
    static async performSearchByResourceType(resource_type, params, perfId, apiStart) {
        const { query, filters, limit, offset, search_type, fields, match_type, sort, } = params;
        switch (resource_type) {
            case UniversalResourceType.COMPANIES:
                return this.searchCompanies(query, filters, limit, offset, search_type, fields, match_type, sort);
            case UniversalResourceType.PEOPLE:
                return this.searchPeople(query, filters, limit, offset, search_type, fields, match_type, sort);
            ...
            case UniversalResourceType.NOTES:
                return this.searchNotes(perfId, apiStart, query, filters, limit, offset);
            default:
     ...
     */
    static async searchCompanies(query, filters, limit, offset, search_type = SearchType.BASIC, fields, match_type = MatchType.PARTIAL, sort = SortType.NAME) {
     ...
     */
    static async searchPeople(query, filters, limit, offset, search_type = SearchType.BASIC, fields, match_type = MatchType.PARTIAL, sort = SortType.NAME) {
        if (filters && Object.keys(filters).length > 0) {
            ...
            if (search_type === SearchType.CONTENT) {
                ...
                // Apply relevance ranking if requested
                if (sort === SortType.RELEVANCE) {
                    return this.rankByRelevance(paginatedResult.results, query, searchFields);
                }
                return paginatedResult.results;
            }
            else {
            ...
            try {
                ...
                const paginatedResult = await searchFn({ filters: [] }, { limit, offset });
                return paginatedResult.results;
            }
            catch (error) {
                // If empty filters aren't supported, return empty array rather than failing
     ...
     */
    static async searchLists(query, limit, offset) {
     ...
     */
    static async searchRecords_ObjectType(limit, offset, filters) {
        ...
        if (filters?.list_membership) {
            ...
            console.warn('list_membership filter not yet supported in listObjectRecords');
        }
     ...
     */
    static async searchDeals(limit, offset) {
     ...
     */
    static async queryDealRecords(params) {
     ...
     */
    static async searchTasks(perfId, apiStart, query, limit, offset) {
        /**
     ...
     */
    static async searchNotes(perfId, apiStart, query, filters, limit, offset) {
        try {
            ...
            const normalizedNotes = notes.map((note) => normalizeNoteResponse(note));
            ...
            if (query && query.trim()) {
                ...
                results = normalizedNotes.filter((record) => {
                    ...
                    const contentMarkdown = record.values?.content_markdown?.toString()?.toLowerCase() || '';
                    const contentPlaintext = record.values?.content_plaintext?.toString()?.toLowerCase() || '';
            ...
            enhancedPerformanceTracker.markTiming(perfId, 'serialization', performance.now() - apiStart);
            return results;
        }
        catch (error) {
            console.error('Failed to search notes:', error);
            // Log error metrics
            enhancedPerformanceTracker.markTiming(perfId, 'other', performance.now() - apiStart);
     ...
     */
    static async getSearchSuggestions(resource_type, partialQuery, limit = 5) {
     ...
     */
    static async getRecordCount(resource_type, filters) {
     ...
     */
    static supportsAdvancedFiltering(resource_type) {
        switch (resource_type) {
            case UniversalResourceType.COMPANIES:
            case UniversalResourceType.PEOPLE:
                return true;
     ...
     */
    static supportsQuerySearch(resource_type) {
        switch (resource_type) {
            case UniversalResourceType.COMPANIES:
            case UniversalResourceType.PEOPLE:
            case UniversalResourceType.LISTS:
     ...
     */
    static rankByRelevance(results, query, searchFields) {
