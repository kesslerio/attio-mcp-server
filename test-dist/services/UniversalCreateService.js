...
import { mapRecordFields, validateResourceType, getFieldSuggestions, validateFields, getValidResourceTypes, FIELD_MAPPINGS, } from '../handlers/tool-configs/universal/field-mapper.js';
...
import { ErrorTemplates, ErrorEnhancer, } from '../errors/enhanced-api-errors.js';
...
function filterAllowedFields(input, allowedFields) {
    return Object.fromEntries(Object.entries(input).filter(([key]) => allowedFields.includes(key)));
}
 ...
 */
async function createCompanyWithMockSupport(companyData) {
 ...
 * Moved to production-side service to avoid test directory imports (Issue #489 Phase 1)
 */
async function createPersonWithMockSupport(personData) {
 ...
 * Moved to production-side service to avoid test directory imports (Issue #489 Phase 1)
 */
async function createTaskWithMockSupport(taskData) {
    // Delegate to production MockService to avoid TypeScript build errors
    const { MockService } = await import('./MockService.js');
 ...
 */
async function enhanceUniquenessError(resourceType, errorMessage, mappedData) {
 ...
 */
function pickAllowedPersonFields(input) {
    const out = {};
    // Core required field
    if (input.name)
        out.name = input.name;
...
export class UniversalCreateService {
    ...
    static async createRecord(params) {
        ...
        console.error('[UniversalCreateService.createRecord] DEBUG - Entry point:', {
            resource_type,
        ...
        console.error('[UniversalCreateService.createRecord] DEBUG - Field validation result:', {
        ...
        if (process.env.ENABLE_ENHANCED_VALIDATION === 'true') {
            const validation = await validateRecordFields(resource_type, mappedData, false);
     ...
     */
    static async createCompanyRecord(mappedData, resource_type) {
        try {
            ...
            if (!result) {
                throw new UniversalValidationError('Company creation failed: createCompany returned null/undefined', ErrorType.API_ERROR, {
                    field: 'result',
                    suggestion: 'Check API connectivity and data format',
                });
            }
            if (!result.id || !result.id.record_id) {
                throw new UniversalValidationError(`Company creation failed: Invalid record structure. Missing ID: ${JSON.stringify(result)}`, ErrorType.API_ERROR, {
                    field: 'id',
            ...
            if (errorMessage.includes('Cannot find attribute')) {
                ...
                if (match && match[1]) {
                    ...
                    throw new UniversalValidationError(enhancedError, ErrorType.USER_ERROR, { suggestion, field: match[1] });
                }
            }
            ...
            if (errorMessage.includes('uniqueness constraint')) {
                ...
                throw new UniversalValidationError(enhancedMessage, ErrorType.USER_ERROR, {
     ...
     */
    static async createListRecord(mappedData, resource_type) {
        try {
     ...
     */
    static async createPersonRecord(mappedData, resource_type) {
        try {
            ...
            if (!result) {
                throw new UniversalValidationError('Person creation failed: createPerson returned null/undefined', ErrorType.API_ERROR, {
                    field: 'result',
                    suggestion: 'Check API connectivity and data format',
                });
            }
            if (!result.id || !result.id.record_id) {
                throw new UniversalValidationError(`Person creation failed: Invalid record structure. Missing ID: ${JSON.stringify(result)}`, ErrorType.API_ERROR, {
                    field: 'id',
                    suggestion: 'Verify API response format and record creation',
                });
            }
     ...
     */
    static async createObjectRecord(mappedData, resource_type) {
        try {
     ...
     */
    static async createDealRecord(mappedData, originalRecordData) {
        ...
        try {
            ...
            if (errorMessage.includes('Cannot find Status') && dealData.stage) {
                ...
                if (defaults.stage) {
                    ...
                    dealData = await applyDealDefaultsWithValidation({ ...originalRecordData, stage: defaults.stage }, true // Skip validation in error path
                    );
                }
                else {
     ...
     */
    static async createTaskRecord(mappedData) {
        try {
            ...
            const content = (mappedData.content &&
            ...
            if (mappedData.title !== undefined && mappedData.content === undefined) {
                throw ErrorTemplates.TASK_FIELD_MAPPING('title', // Show 'title' as the field they tried to use
            ...
            const recordId = mappedData.linked_records ||
            ...
            debug('universal.createTask', 'About to convert task to record', {
            ...
            if ('values' in createdTask && createdTask.id?.record_id) {
                ...
                if ('content' in createdTask) {
                    convertedRecord = UniversalUtilityService.convertTaskToRecord(createdTask);
                }
                else {
                    throw new Error(`Invalid task object structure: ${JSON.stringify(createdTask)}`);
                }
            }
            // Debug logging after conversion
            debug('universal.createTask', 'Task converted to record', {
     ...
     */
    static async createNoteRecord(mappedData) {
        try {
            ...
            if (typeof mappedData.content === 'string' && mappedData.content.trim()) {
                ...
                throw new UniversalValidationError('Content is required and must be a non-empty string', ErrorType.USER_ERROR, { field: 'content' });
            }
            // Validate parent_object (after field mapping)
            const parentObject = mappedData.parent_object;
            if (!parentObject || !['companies', 'people'].includes(parentObject)) {
                throw new UniversalValidationError('parent_object must be "companies" or "people"', ErrorType.USER_ERROR, { field: 'parent_object' });
            }
            // Validate parent_record_id (after field mapping)
            const parentRecordId = mappedData.parent_record_id;
            if (!parentRecordId) {
            ...
            const noteBody = {
                ...
                format: mappedData.format || 'plaintext',
                created_at: mappedData.created_at,
                meeting_id: mappedData.meeting_id,
            };
            debug('universal.createNote', 'Creating note with mapped data', {
            ...
            }, 'createNote', OperationType.API_CALL);
            ...
            debug('universal.createNote', 'Note created and normalized', {
            ...
            const errorObj = error instanceof Error ? error : new Error(String(error));
            const enhancedError = ErrorEnhancer.autoEnhance(errorObj, 'notes', 'create-record');
            throw enhancedError;
        }
    }
    /**
