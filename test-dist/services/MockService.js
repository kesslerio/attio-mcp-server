        ...
        return {
            ...
            values: {
                ...
                domains: Array.isArray(companyData.domains)
                    ? companyData.domains.map((d) => ({ value: d }))
                    : [{ value: `${mockId}.example.com` }],
                industry: [{ value: companyData.industry || 'Technology' }],
                description: [
                    {
    ...
    static async createPerson(personData) {
        ...
        return {
            ...
            values: {
                name: [
                    {
                        value: personData.name || `Mock Person ${mockId.slice(-4)}`,
                    },
                ],
                email_addresses: Array.isArray(personData.email_addresses)
                    ? personData.email_addresses.map((email) => ({
                        value: email,
                    }))
                    : [{ value: `${mockId}@example.com` }],
            },
    ...
    static async updateTask(taskId, updateData) {
        ...
        const attioRecord = {
            ...
            values: {
                content: [{ value: taskContent }],
                title: [{ value: taskContent }], // Issue #480: Dual field support
                status: [{ value: updateData.status || 'updated' }],
