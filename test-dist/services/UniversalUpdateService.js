...
import { mapRecordFields, validateResourceType, getFieldSuggestions, validateFields, getValidResourceTypes, mapTaskFields, } from '../handlers/tool-configs/universal/field-mapper.js';
 ...
 */
async function updateTaskWithMockSupport(taskId, updateData) {
...
export class UniversalUpdateService {
    ...
    static async updateRecord(params) {
        ...
        if (mappingResult.errors && mappingResult.errors.length > 0) {
            throw new UniversalValidationError(mappingResult.errors.join(' '), ErrorType.USER_ERROR, {
                field: 'record_data',
        ...
        if (process.env.ENABLE_ENHANCED_VALIDATION === 'true') {
            const validation = await validateRecordFields(resource_type, sanitizedData, false);
        ...
        switch (resource_type) {
            case UniversalResourceType.COMPANIES:
                updatedRecord = await this.updateCompanyRecord(record_id, sanitizedData, resource_type);
                break;
            case UniversalResourceType.LISTS:
                updatedRecord = await this.updateListRecord(record_id, sanitizedData, resource_type);
                break;
            case UniversalResourceType.PEOPLE:
                updatedRecord = await this.updatePersonRecord(record_id, sanitizedData, resource_type);
                break;
            case UniversalResourceType.RECORDS:
                updatedRecord = await updateObjectRecord('records', record_id, sanitizedData);
                break;
            ...
            case UniversalResourceType.TASKS:
                ...
                break;
            default:
                updatedRecord = await this.handleUnsupportedResourceType(resource_type, params);
                break;
        }
        // Normalize response format across all resource types (Issue #473)
        const normalizedRecord = this.normalizeResponseFormat(resource_type, updatedRecord);
        // Verify field persistence after successful update (Issue #473)
        if (process.env.ENABLE_FIELD_VERIFICATION !== 'false') {
            try {
                const verification = await this.verifyFieldPersistence(resource_type, record_id, sanitizedData, normalizedRecord);
                if (verification.warnings.length > 0) {
                    console.error(`Field persistence warnings for ${resource_type} ${record_id}:`, verification.warnings);
                }
                if (!verification.verified) {
                    console.warn(`Field persistence verification failed for ${resource_type} ${record_id}:`, verification.discrepancies);
     ...
     */
    static async updateCompanyRecord(record_id, mappedData, resource_type) {
        try {
            ...
            if (errorMessage.includes('Cannot find attribute')) {
                ...
                if (match && match[1]) {
                    ...
                    throw new UniversalValidationError(error.message, ErrorType.USER_ERROR, { suggestion, field: match[1] });
                }
            }
            throw error;
        }
     ...
     */
    static async updateListRecord(record_id, mappedData, resource_type) {
        try {
     ...
     */
    static async updatePersonRecord(record_id, mappedData, resource_type) {
        try {
     ...
     */
    static async updateDealRecord(record_id, mappedData) {
     ...
     */
    static async updateTaskRecord(record_id, mappedData) {
        ...
        if (mappedData.linked_records !== undefined) {
            ...
            if (Array.isArray(mappedData.linked_records)) {
                taskUpdateData.recordIds = mappedData.linked_records.map((link) => link.record_id || link.id || link);
            }
            else {
        ...
        return shouldUseMockData()
            ...
            : UniversalUtilityService.convertTaskToRecord(updatedTask);
    }
    /**
     * Validate that task content fields are not being updated (immutable)
     */
    static assertNoTaskContentUpdate(record_data) {
     ...
     */
    static async handleUnsupportedResourceType(resource_type, params) {
        ...
        if (resourceValidation.corrected) {
            ...
            console.error(`Resource type corrected from "${resource_type}" to "${resourceValidation.corrected}"`);
            return this.updateRecord({
                ...params,
                resource_type: resourceValidation.corrected,
            });
        }
        throw new UniversalValidationError(`Unsupported resource type: ${resource_type}`, ErrorType.USER_ERROR, {
            suggestion: resourceValidation.suggestion ||
     ...
     */
    static sanitizeSpecialCharacters(data) {
        const sanitized = {};
        for (const [key, value] of Object.entries(data)) {
            if (typeof value === 'string') {
                ...
                sanitized[key] = value.map((item) => (typeof item === 'string' ? item : item) // Keep strings as-is for now
                );
            }
            else if (value && typeof value === 'object') {
                // Recursively sanitize nested objects
                sanitized[key] = this.sanitizeSpecialCharacters(value);
            }
            else {
     ...
     */
    static async verifyFieldPersistence(resource_type, record_id, expectedUpdates, updatedRecord) {
        const result = {
            verified: true,
            discrepancies: [],
            warnings: [],
        ...
        try {
            ...
            const verificationRecord = await this.fetchRecordForVerification(resource_type, record_id);
            ...
            for (const [fieldName, expectedValue] of Object.entries(expectedUpdates)) {
                ...
                if (!comparisonResult.matches) {
                    result.verified = false;
                    result.discrepancies.push(`Field "${fieldName}" persistence mismatch: expected ${JSON.stringify(expectedValue)}, got ${JSON.stringify(actualValue)}`);
                }
            ...
            if (result.discrepancies.length > 0) {
                console.warn(`Field persistence verification failed for ${resource_type} ${record_id}:`, result.discrepancies);
            }
            else if (result.warnings.length > 0) {
                console.error(`Field persistence verification completed with warnings for ${resource_type} ${record_id}:`, result.warnings);
            }
        }
        catch (error) {
     ...
     */
    static async fetchRecordForVerification(resource_type, record_id) {
        try {
            switch (resource_type) {
                case UniversalResourceType.COMPANIES:
                ...
                case UniversalResourceType.DEALS:
                case UniversalResourceType.RECORDS:
                    return await getObjectRecord(resource_type === UniversalResourceType.DEALS ? 'deals' : 'records', record_id);
                default:
                    console.warn(`No verification method available for resource type: ${resource_type}`);
                    return null;
            }
        }
        catch (error) {
            console.error(`Failed to fetch ${resource_type} record ${record_id} for verification:`, error);
            return null;
        }
    }
    /**
     ...
     */
    static compareFieldValues(fieldName, expectedValue, actualValue) {
        ...
        if (Array.isArray(expectedValue)) {
            ...
            const actualSet = new Set(unwrappedActual.map((v) => String(v)));
            return {
                matches: expectedSet.size === actualSet.size &&
     ...
     */
    static normalizeResponseFormat(resource_type, record) {
        // Ensure the record has the required AttioRecord structure
        if (!record || typeof record !== 'object') {
            throw new Error(`Invalid record format received for ${resource_type}: ${typeof record}`);
        }
        ...
        switch (resource_type) {
            ...
            default:
                console.warn(`No specific normalization available for resource type: ${resource_type}`);
                return normalizedRecord;
        }
    }
    /**
     ...
     */
    static normalizeCompanyRecord(record) {
        ...
        return {
            ...
            values: {
                ...
                domains: record.values.domains && Array.isArray(record.values.domains)
     ...
     */
    static normalizePersonRecord(record) {
        return {
            ...record,
            id: {
                ...record.id,
            ...
            values: {
                ...
                email_addresses: record.values.email_addresses &&
                    Array.isArray(record.values.email_addresses)
                    ? record.values.email_addresses
                    : record.values.email_addresses
                        ? [record.values.email_addresses]
                        : record.values.email_addresses,
                phone_numbers: record.values.phone_numbers &&
     ...
     */
    static normalizeListRecord(record) {
        return {
            ...record,
            id: {
                ...record.id,
     ...
     */
    static normalizeTaskRecord(record) {
        return {
            ...record,
            id: {
                ...record.id,
     ...
     */
    static normalizeDealRecord(record) {
        return {
            ...record,
            id: {
                ...record.id,
            ...
            values: {
                ...
                value: record.values.value && typeof record.values.value === 'string'
     ...
     */
    static normalizeGenericRecord(record) {
        return {
            ...record,
            id: {
                ...record.id,
