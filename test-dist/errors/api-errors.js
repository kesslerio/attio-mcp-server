...
export class AuthenticationError extends AttioApiError {
    constructor(message = 'Authentication failed. Please check your credentials.', endpoint, method, details) {
        // Sanitize the message to avoid exposing API key format
        const sanitizedMessage = message.replace(/api[_-]?key[\s:=]*["']?[a-zA-Z0-9_-]{20,}["']?/gi, '[CREDENTIAL_REDACTED]');
...
export class AuthorizationError extends AttioApiError {
    constructor(message = 'Authorization failed. You lack the necessary permissions.', endpoint, method, details) {
...
export class ResourceNotFoundError extends AttioApiError {
    constructor(resourceType, resourceId, endpoint, method, details) {
        ...
        super(`${resourceType} ${sanitizedId} not found`, 404, endpoint, method, details);
