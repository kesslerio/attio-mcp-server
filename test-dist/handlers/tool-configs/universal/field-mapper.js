 ...
 */
export function mapFieldName(resourceType, fieldName) {
...
export function detectFieldCollisions(resourceType, recordData) {
    ...
    for (const [targetField, inputFields] of Object.entries(targetToInputs)) {
        if (inputFields.length > 1) {
            ...
            errors.push(`Field collision detected: ${inputFieldsList} all map to "${targetField}". ` +
 ...
 */
function getFieldCollisionSuggestion(resourceType, targetField, conflictingFields) {
    const mapping = FIELD_MAPPINGS[resourceType];
    if (!mapping)
        return '';
 ...
 */
export function mapRecordFields(resourceType, recordData) {
    const mapping = FIELD_MAPPINGS[resourceType];
    if (!mapping) {
...
export function validateResourceType(resourceType) {
    ...
    return {
        valid: false,
        suggestion: `Invalid resource type "${resourceType}". Valid types are: ${validTypes.join(', ')}${suggestions.length > 0
 ...
 */
export function getFieldSuggestions(resourceType, fieldName) {
    const mapping = FIELD_MAPPINGS[resourceType];
    if (!mapping) {
 ...
 */
export function validateFields(resourceType, recordData) {
    const mapping = FIELD_MAPPINGS[resourceType];
    if (!mapping) {
        return {
            valid: true,
    ...
    if (mapping.requiredFields) {
        for (const required of mapping.requiredFields) {
            if (!(required in recordData)) {
                ...
                const hasMappedVersion = Object.keys(recordData).some((key) => mapFieldName(resourceType, key) === required);
                if (!hasMappedVersion) {
 ...
 */
export async function enhanceUniquenessError(resourceType, errorMessage, recordData) {
    const mapping = FIELD_MAPPINGS[resourceType];
    ...
    try {
        ...
        const attribute = attributes.find((attr) => attr.id === attributeId || attr.api_slug === attributeId);
        if (attribute) {
 ...
 */
function findSimilarStrings(input, candidates, threshold = 0.6) {
    const similarities = [];
...
export function validateCategories(input) {
    ...
    if (typeof input === 'string') {
        ...
        result.errors.push('Categories must be a string or array of strings. Use ["category"] format for arrays.');
        return result;
    }
    ...
    for (const category of categories) {
        ...
        const exactMatch = VALID_COMPANY_CATEGORIES.find((valid) => valid.toLowerCase() === category.toLowerCase());
        if (exactMatch) {
            ...
            result.isValid = false;
            result.errors.push(`Invalid category "${category}". ${suggestions.length > 0
 ...
 */
export function processCategories(resourceType, fieldName, value) {
    const result = {
    ...
    if (validation.autoConverted) {
        result.warnings.push(`Categories field auto-converted from string to array format: ["${value}"]`);
    }
    if (!validation.isValid) {
        result.errors.push(...validation.errors);
        // Always show valid options when there are errors
        result.warnings.push(`Valid category options (first 10): ${VALID_COMPANY_CATEGORIES.slice(0, 10).join(', ')}`);
    }
    else {
        result.processedValue = validation.validatedCategories;
        if (validation.autoConverted) {
            result.warnings.push(`Category value successfully validated and converted to: ${JSON.stringify(validation.validatedCategories)}`);
        }
    }
    return result;
}
 ...
 */
export function mapTaskFields(operation, input) {
    const output = { ...input };
