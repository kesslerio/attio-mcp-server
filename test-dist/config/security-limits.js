...
export const LIMIT_ERROR_MESSAGES = {
    BATCH_SIZE_EXCEEDED: (size, limit, operation) => `Batch size (${size}) exceeds maximum allowed (${limit})${operation ? ` for ${operation}` : ''}. ` +
        `Please split into smaller batches for security and performance.`,
    PAYLOAD_SIZE_EXCEEDED: (size, limit) => `Request payload size (${formatBytes(size)}) exceeds maximum allowed (${formatBytes(limit)}). ` +
        `Please reduce the amount of data in your request.`,
    SINGLE_RECORD_SIZE_EXCEEDED: (size, limit) => `Single record size (${formatBytes(size)}) exceeds maximum allowed (${formatBytes(limit)}). ` +
        `Please reduce the amount of data in this record.`,
    SEARCH_QUERY_TOO_LONG: (length, limit) => `Search query length (${length} characters) exceeds maximum allowed (${limit} characters). ` +
        `Please use a shorter search query.`,
    FILTER_TOO_COMPLEX: (size, limit) => `Filter object size (${formatBytes(size)}) exceeds maximum allowed (${formatBytes(limit)}). ` +
        `Please simplify your filter criteria.`,
    RATE_LIMIT_EXCEEDED: (requests, limit) => `Request rate (${requests} requests) exceeds maximum allowed (${limit} per minute). ` +
 ...
 */
export function getBatchSizeLimit(resourceType) {
    if (!resourceType)
        return BATCH_SIZE_LIMITS.DEFAULT;
    const upperType = resourceType.toUpperCase();
    return (BATCH_SIZE_LIMITS[upperType] ||
