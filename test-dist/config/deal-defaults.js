 ...
 */
export function clearDealCaches() {
    stageCache = null;
 ...
 */
export async function prewarmStageCache() {
    try {
 ...
 */
export function getDealDefaults() {
    return {
 ...
 */
export function applyDealDefaults(recordData) {
 ...
 */
export function validateDealInput(recordData) {
    const errors = [];
    const warnings = [];
    const suggestions = [];
    // Check for common field name mistakes
    if (recordData.company_id) {
        suggestions.push('Use "associated_company" instead of "company_id" for linking to companies');
    }
    if (recordData.company) {
        suggestions.push('Use "associated_company" instead of "company" for linking to companies');
    }
    ...
    if (recordData.stage &&
        ...
        if (!recordData.stage[0]?.status) {
            ...
            suggestions.push('Stage should be: [{"status": "stage_name"}] or just "stage_name"');
        }
    }
    // Check owner format
    if (recordData.owner && typeof recordData.owner === 'string') {
        warnings.push('Owner should be in proper format for workspace member reference');
        suggestions.push('Owner will be auto-formatted to proper workspace member reference');
    }
    return {
 ...
 */
async function getAvailableDealStages() {
    const now = Date.now();
    ...
    try {
        ...
        const stageAttribute = attributes.find((attr) => attr.api_slug === 'stage');
        if (!stageAttribute) {
            return [];
        }
 ...
 */
export async function validateDealStage(stage, skipApiCall = false) {
    if (!stage) {
        return undefined;
    }
    try {
        ...
        const validStage = availableStages.find((s) => s.toLowerCase() === stage.toLowerCase());
        if (validStage) {
            return validStage; // Return the correctly cased version
        }
        // Stage not found, log warning and return default
        const defaults = getDealDefaults();
        console.error(`Deal stage "${stage}" not found. Available stages: ${availableStages.join(', ')}. Using default: "${defaults.stage}"`);
 ...
 */
export async function applyDealDefaultsWithValidation(recordData, skipValidation = false) {
    ...
    if (dealData.stage &&
        ...
        const validatedStage = await validateDealStage(dealData.stage[0].status, skipValidation // Skip API calls when in error paths
        );
        if (validatedStage) {
