...
export function verifyToolConfigsWithRequiredTools(resourceName: string, combinedConfigs: any, requiredToolTypes: string[]): boolean {
  ...
  for (const toolType of requiredToolTypes) {
    if (toolType in combinedConfigs) {
      ...
      console.log(`[${resourceName}ToolConfigs] Found ${toolType} with name: ${config.name}`);
      
      // Additional validation
      if (typeof config.handler !== 'function') {
        console.warn(`[${resourceName}ToolConfigs] WARNING: ${toolType} has no handler function!`);
        allPresent = false;
      }
      
      if (typeof config.formatResult !== 'function' && toolType !== 'create' && toolType !== 'update') {
        console.warn(`[${resourceName}ToolConfigs] WARNING: ${toolType} has no formatResult function!`);
      }
    } else {
      console.warn(`[${resourceName}ToolConfigs] MISSING: Required tool type '${toolType}' not found!`);
      allPresent = false;
    }
  }
  ...
  if (debugMode) {
    console.log(`[${resourceName}ToolConfigs] All registered tool types:`, Object.keys(combinedConfigs));
  }
...
export function verifySpecificTool(resourceName: string, configs: any, toolType: string, subConfigs: any = null): boolean {
  ...
  if (subConfigs && toolType in subConfigs) {
    console.log(`[${resourceName}ToolConfigs] Found ${toolType} in sub-configs`);
    
    // Check if the tool was properly merged into the main configs
    if (toolType in configs) {
      return true;
    } else {
      console.warn(`[${resourceName}ToolConfigs] WARNING: ${toolType} exists in sub-config but not in combined config!`);
      return false;
    }
  }
  ...
  console.warn(`[${resourceName}ToolConfigs] WARNING: ${toolType} not found in configs!`);
  return false;
}