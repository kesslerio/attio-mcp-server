  ...
  try {
    ...
    if (toolType === 'notes') {
      ...
      try {
        ...
        const notes = await notesToolConfig.handler(
          notesTargetId,
          limit,
          offset
        );
    ...
    if (toolType === 'createNote') {
      ...
      try {
        ...
        const note = await createNoteToolConfig.handler(
          noteTargetId,
          title,
          content
        );
    ...
    if (toolType === 'getListEntries') {
      ...
      if (
        request.params.arguments?.offset !== undefined &&
        request.params.arguments?.offset !== null
      ) {
    ...
    if (toolType === 'filterListEntries') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          '[filterListEntries] Processing request with parameters:',
          {
            listId,
            attributeSlug,
            condition,
      ...
      try {
        const entries = await toolConfig.handler(
          listId,
          attributeSlug,
          condition,
          value,
    ...
    if (toolType === 'advancedFilterListEntries') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          '[advancedFilterListEntries] Processing request with parameters:',
          {
            listId,
    ...
    if (toolType === 'updateListEntry') {
      const listId = request.params.arguments?.listId as string;
      const entryId = request.params.arguments?.entryId as string;
      const attributes = request.params.arguments?.attributes as Record<
        string,
        any
      >;
    ...
    async function handleCompanyBatchOperation(
      ...
      logToolRequest(
        operation === 'create' ? 'batchCreate' : 'batchUpdate',
        operation === 'create'
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        console.error(
          '- Arguments has paramName:',
          request.params.arguments && paramName in request.params.arguments
        );
      }
      ...
      if (!Array.isArray(records)) {
        return createErrorResult(
          ...
          {
            status: 400,
            message: `Invalid parameter: ${paramName} must be an array`,
          }
        );
      }
      ...
      if (operation === 'update') {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (!record || typeof record !== 'object') {
            return createErrorResult(
              new Error(
                `Invalid update data at index ${i}: must be a non-null object`
              ),
              `/objects/companies/records/batch`,
              httpMethod,
              {
                status: 400,
                message: `Invalid update data at index ${i}: must be a non-null object`,
              }
            );
          }
          ...
          if (!record.id) {
            return createErrorResult(
              ...
              {
                status: 400,
                message: `Invalid update data at index ${i}: 'id' is required`,
              }
            );
          }
          ...
          if (attributesValidation !== true) {
            return createErrorResult(
              new Error(
                `Invalid update data at index ${i}: ${attributesValidation}`
              ),
              `/objects/companies/records/batch`,
              httpMethod,
              {
                status: 400,
                message: `Invalid update data at index ${i}: ${attributesValidation}`,
              }
            );
          }
        }
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (!record || typeof record !== 'object') {
            return createErrorResult(
              new Error(
                `Invalid company data at index ${i}: must be a non-null object`
              ),
              `/objects/companies/records/batch`,
              httpMethod,
              {
                status: 400,
                message: `Invalid company data at index ${i}: must be a non-null object`,
              }
            );
          }
          ...
          if (!record.name) {
            return createErrorResult(
              new Error(
                `Invalid company data at index ${i}: 'name' is required`
              ),
              `/objects/companies/records/batch`,
              httpMethod,
              {
                status: 400,
                message: `Invalid company data at index ${i}: 'name' is required`,
              }
            );
          }
        }
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.error(
            `[handleCompanyBatchOperation:${operation}] Calling handler with params:`,
            JSON.stringify(params, null, 2)
          );
          console.error(
            '- Handler function:',
            toolConfig.handler.name || 'anonymous'
          );
        }

        const result = await toolConfig.handler(params);
        return formatResponse(
          formatBatchResults(result, operation),
    ...
    if (toolType === 'batchCreate' && toolName === 'batch-create-companies') {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        console.error(
          '- Handler exists:',
          toolConfig && typeof toolConfig.handler === 'function'
        );
      }
    ...
    async function handleCompanyOperation(
      ...
      const logName =
        operation === 'create' ? 'createCompany' : 'updateCompany';
      const httpMethod = operation === 'create' ? 'POST' : 'PATCH';
    ...
    if (
      ...
      return await executeRecordOperation(
        toolType,
        toolConfig,
        request,
        resourceType
    ...
    if (
      ...
      return await executeAdvancedSearch(
        toolType,
        toolConfig,
        request,
        resourceType
    ...
    if (
      ['searchByCompany', 'searchByCompanyList', 'searchByNotes'].includes(
        toolType
      )
    ) {
      return await executeRelationshipSearch(
        toolType,
        toolConfig,
        request,
        resourceType
    ...
    if (toolType === 'getAttributes') {
      ...
      const idOrError = validateResourceId(
        resourceType,
        request.params.arguments,
        apiPath
      );
    ...
    if (toolType === 'discoverAttributes') {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        console.error(
          '- Tool handler exists:',
          typeof toolConfig.handler === 'function'
        );
        console.error(
          '- Tool formatter exists:',
          typeof toolConfig.formatResult === 'function'
        );
      }
      ...
      try {
        ...
        return createErrorResult(
          error instanceof Error
            ? error
            : new Error(
                `Unknown error in discoverAttributes: ${String(error)}`
              ),
          apiPath,
          'GET',
          hasResponseData(error) ? error.response.data : {}
    ...
    async function handleCompanyInfoTool(
      ...
      const formattedToolType = toolType
      ...
      if (extraParams?.requireFields) {
        ...
        if (!fields || !Array.isArray(fields) || fields.length === 0) {
          return createErrorResult(
            new Error(
              'fields parameter is required and must be a non-empty array'
            ),
            apiPath,
            'GET',
    ...
    if (companyInfoTools.includes(toolType)) {
      return await handleCompanyInfoTool(
        resourceType,
        toolType,
        toolConfig,
        request
    ...
    async function handleCompanyAttributeUpdate(
      ...
      if (
        ...
        return createErrorResult(
          new Error(
            'attributeName parameter is required and must be a non-empty string'
          ),
          apiPath,
          'PATCH',
          {
            status: 400,
            message:
              'Missing or invalid required parameter: attributeName must be a non-empty string',
          }
        );
      }
      ...
      if (request.params.arguments && !('value' in request.params.arguments)) {
        return createErrorResult(
          new Error(
            'value parameter is required (use null to clear an attribute)'
          ),
          apiPath,
          'PATCH',
          {
            status: 400,
            message:
              'Missing required parameter: value (use null to clear an attribute)',
          }
        );
      }
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[updateAttribute] Updating attribute for ${resourceType} ${id}:`,
          {
            attributeName,
    ...
    if (
      toolType === 'updateAttribute' &&
      toolName === 'update-company-attribute'
    ) {
      return await handleCompanyAttributeUpdate(
    ...
    if (toolType === 'json') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[json] Processing JSON tool request for ${resourceType}:`
        );
        console.error('- Tool name:', toolName);
        console.error(
          '- Request arguments:',
...
async function executeRecordOperation(
  ...
  const objectSlug =
    (request.params.arguments?.objectSlug as string) || resourceType;
  const objectId = request.params.arguments?.objectId as string;

  // For debugging
  if (process.env.NODE_ENV === 'development') {
    console.error(
      `[executeRecordOperation] Processing ${toolType} operation for ${resourceType}`
    );
    console.error(`[executeRecordOperation] Object slug: ${objectSlug}`);
    console.error(
      `[executeRecordOperation] Request arguments:`,
      JSON.stringify(request.params.arguments, null, 2)
    );
  }
  ...
  if (toolType === 'create') {
    const recordData =
      request.params.arguments?.recordData ||
      request.params.arguments?.attributes;

    try {
      const recordCreateConfig = toolConfig as RecordCreateToolConfig;
      const record = await recordCreateConfig.handler(
        objectSlug,
        recordData,
        objectId
      );
  ...
  if (toolType === 'batchCreate') {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[batchCreate] Processing request for ${resourceType || objectSlug}:`,
          {
            resourceType,
            objectSlug,
      ...
      if (resourceType === ResourceType.COMPANIES) {
        ...
        if (!Array.isArray(companies)) {
          return createErrorResult(
            ...
            {
              status: 400,
              message: "Invalid 'companies' parameter: expected array",
            }
          );
        }
        ...
        if (companies.length === 0) {
          return createErrorResult(
            ...
            {
              status: 400,
              message: "Invalid 'companies' parameter: array cannot be empty",
            }
          );
        }
        ...
        return formatResponse(
          formatBatchResults(result, 'create'),
          result.summary.failed > 0
        );
      } else {
        ...
        if (!Array.isArray(records)) {
          return createErrorResult(
            ...
            {
              status: 400,
              message: "Invalid 'records' parameter: expected array",
            }
          );
        }
        ...
        if (records.length === 0) {
          return createErrorResult(
            ...
            {
              status: 400,
              message: "Invalid 'records' parameter: array cannot be empty",
            }
          );
        }

        const recordBatchCreateConfig =
          toolConfig as RecordBatchCreateToolConfig;
        const result = await recordBatchCreateConfig.handler(
          objectSlug,
          records,
          objectId
        );
      ...
      console.error(
        `[batchCreate] Error executing batch create for ${resourceType || objectSlug}:`,
        error
      );
  ...
  if (toolType === 'batchUpdate') {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[batchUpdate] Processing request for ${resourceType || objectSlug}:`,
          {
            resourceType,
            objectSlug,
            arguments: request.params.arguments,
      ...
      if (resourceType === ResourceType.COMPANIES) {
        ...
        if (!Array.isArray(updates)) {
          return createErrorResult(
            ...
            {
              status: 400,
              message: "Invalid 'updates' parameter: expected array",
            }
          );
        }
        ...
        if (updates.length === 0) {
          return createErrorResult(
            ...
            {
              status: 400,
              message: "Invalid 'updates' parameter: array cannot be empty",
            }
          );
        }
        ...
        for (let i = 0; i < updates.length; i++) {
          ...
          if (!update.attributes || typeof update.attributes !== 'object') {
            return createErrorResult(
              new Error(
                `Invalid update data at index ${i}: 'attributes' must be a non-null object`
              ),
              `objects/${ResourceType.COMPANIES}/records/batch`,
              'PATCH',
              {
                status: 400,
                message: `Invalid update data at index ${i}: 'attributes' must be a non-null object`,
              }
            );
          }
        }
        ...
        // Format the response based on success/failure
        return formatResponse(
          formatBatchResults(result, 'update'),
          result.summary.failed > 0
        );
      } else {
        ...
        const recordBatchUpdateConfig =
          toolConfig as RecordBatchUpdateToolConfig;
        const result = await recordBatchUpdateConfig.handler(
          objectSlug,
          records,
          objectId
        );
      ...
      // Enhanced error handling for batch operations
      console.error(
        `[batchUpdate] Error executing batch update for ${resourceType || objectSlug}:`,
        error
      );
  ...
  if (toolType === 'get') {
    ...
    if (resourceType === ResourceType.COMPANIES) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:get:company] Request arguments:`,
          JSON.stringify(request.params.arguments, null, 2)
        );
        console.error(
          `[executeRecordOperation:get:company] Extracted companyId: ${recordId}`
        );
      }
    ...
    if (!recordId || typeof recordId !== 'string') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:get] Missing or invalid ${idParamName} parameter:`,
      ...
      return createErrorResult(
        new Error(
          `${idParamName} parameter is required and must be a non-empty string`
        ),
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(
        `[executeRecordOperation:get] API endpoint: /objects/${objectSlug}/records/${recordId}`
      );
    }
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:get] Retrieval successful for ID: ${recordId}`
        );
        console.error(
          `[executeRecordOperation:get] Response:`,
          JSON.stringify(
            record,
            (key, value) =>
              key === 'values'
                ? Object.keys(value).length + ' fields retrieved'
                : value,
            2
          )
        );
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:get] Retrieval failed for ID: ${recordId}:`,
          error instanceof Error ? error.message : String(error)
        );

        if (error instanceof Error && error.stack) {
          console.error(
            `[executeRecordOperation:get] Stack trace:`,
            error.stack
          );
        }
      }
      ...
      return createErrorResult(
        error instanceof Error
          ? error
          : new Error(`Failed to get ${objectSlug} record: ${String(error)}`),
  ...
  if (toolType === 'update') {
    ...
    if (resourceType === ResourceType.COMPANIES) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:update:company] Request arguments:`,
          JSON.stringify(request.params.arguments, null, 2)
        );
        console.error(
          `[executeRecordOperation:update:company] Extracted companyId: ${recordId}`
        );
        console.error(
          `[executeRecordOperation:update:company] Tool name: ${request.params.name}`
        );
      }
    } else if (resourceType === ResourceType.PEOPLE) {
    ...
    if (!recordId || typeof recordId !== 'string') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:update] Missing or invalid ${idParamName} parameter:`,
          { resourceType, toolType, requestArgs: request.params.arguments }
        );
      }
    ...
    if (
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:update] Missing or invalid record data:`,
      ...
      return createErrorResult(
        new Error(
          `Record data parameter is required and must be a non-empty object`
        ),
        `objects/${objectSlug}/records/${recordId}`,
        'PATCH',
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(
        `[executeRecordOperation:update] Record data:`,
        JSON.stringify(recordData, null, 2)
      );
      console.error(
        `[executeRecordOperation:update] API endpoint: /objects/${objectSlug}/records/${recordId}`
      );
    }

    try {
      const recordUpdateConfig = toolConfig as RecordUpdateToolConfig;
      const record = await recordUpdateConfig.handler(
        objectSlug,
        recordId,
        recordData
      );
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:update] Update successful for ID: ${recordId}`
        );
        console.error(
          `[executeRecordOperation:update] Response:`,
          JSON.stringify(
            record,
            (key, value) =>
              key === 'values'
                ? Object.keys(value).length + ' fields updated'
                : value,
            2
          )
        );
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:update] Update failed for ID: ${recordId}:`,
          error instanceof Error ? error.message : String(error)
        );

        if (error instanceof Error && error.stack) {
          console.error(
            `[executeRecordOperation:update] Stack trace:`,
            error.stack
          );
        }
      }
      ...
      return createErrorResult(
        error instanceof Error
          ? error
          : new Error(
              `Failed to update ${objectSlug} record: ${String(error)}`
            ),
        `objects/${objectSlug}/records/${recordId}`,
        'PATCH',
        (error as any).response?.data || {}
  ...
  if (toolType === 'delete') {
    ...
    if (resourceType === ResourceType.COMPANIES) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:delete:company] Request arguments:`,
          JSON.stringify(request.params.arguments, null, 2)
        );
        console.error(
          `[executeRecordOperation:delete:company] Extracted companyId: ${recordId}`
        );
      }
    } else if (resourceType === ResourceType.PEOPLE) {
      recordId = request.params.arguments?.personId as string;
    ...
    if (!recordId || typeof recordId !== 'string') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:delete] Missing or invalid ${idParamName} parameter:`,
          { resourceType, toolType, requestArgs: request.params.arguments }
        );
      }
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(
        `[executeRecordOperation:delete] API endpoint: /objects/${objectSlug}/records/${recordId}`
      );
    }
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:delete] Deletion ${success ? 'successful' : 'unsuccessful'} for ID: ${recordId}`
        );
      }
      ...
      if (process.env.NODE_ENV === 'development') {
        console.error(
          `[executeRecordOperation:delete] Deletion failed for ID: ${recordId}:`,
          error instanceof Error ? error.message : String(error)
        );

        if (error instanceof Error && error.stack) {
          console.error(
            `[executeRecordOperation:delete] Stack trace:`,
            error.stack
          );
        }
      }
      ...
      return createErrorResult(
        error instanceof Error
          ? error
          : new Error(
              `Failed to delete ${objectSlug} record: ${String(error)}`
            ),
        `objects/${objectSlug}/records/${recordId}`,
        'DELETE',
        (error as any).response?.data || {}
  ...
  if (toolType === 'list') {
    ...
    try {
      ...
      const records = await recordListConfig.handler(
        objectSlug,
        String(limit),
        String(offset)
      );
      ...
      return createErrorResult(
        error instanceof Error
          ? error
          : new Error(`Failed to list ${objectSlug} records: ${String(error)}`),
        `objects/${objectSlug}/records`,
        'GET',
        (error as any).response?.data || {}
      );
  ...
  throw new Error(
    `Record operation handler not implemented for tool type: ${toolType}`
  );
}
...
async function executeRelationshipSearch(
  ...
  try {
    ...
    const formattedResults = toolConfig.formatResult
      ? toolConfig.formatResult(results)
...
async function executeAdvancedSearch(
  ...
  const advancedSearchConfig = toolConfig as
    | AdvancedSearchToolConfig
    | DateBasedSearchToolConfig;
  let results: AttioRecord[] = [];
  ...
  try {
    ...
    if (
      ...
      if (typeof dateRange === 'string') {
        try {
          ...
          console.warn(
            `Failed to parse dateRange parameter: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }
      ...
      const response = await advancedSearchConfig.handler(
        filter,
        limit,
        offset
      );
    ...
    if (error instanceof ValueMatchError) {
      ...
      errorDetailsForCreateResult = {
        message: (error as Error)?.message || 'Unknown error details',
      };
    }
    ...
    return createErrorResult(
      error instanceof Error
        ? error
        : new Error('Unknown error caught in advancedSearch'),
      `/objects/${resourceType}/records/query`,
      'POST',
      errorDetailsForCreateResult
    );
