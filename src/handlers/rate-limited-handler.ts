...
export function withSearchRateLimiting<T extends any[]>(
  ...
  return async (...args: T) => {
    ...
    try {
      ...
      // Format error for API response
      const errorMessage = error instanceof FilterValidationError ? error.message : 
        (error instanceof Error ? `Unexpected error: ${error.message}` : `Unknown error: ${String(error)}`);
