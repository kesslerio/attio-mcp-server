 ...
 */
function formatPaginatedPeopleResult(result: PaginatedResponse<Person>): string {
  ...
  // Format result list
  const resultList = results.map((person: Person) => 
    `- ${person.values?.name?.[0]?.value || 'Unnamed'} (ID: ${person.id?.record_id || 'unknown'})`
  ).join('\n');
