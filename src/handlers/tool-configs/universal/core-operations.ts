...
export const discoverAttributesConfig: UniversalToolConfig = {
  ...
  formatResult: (schema: unknown, resourceType?: UniversalResourceType): string => {
    if (!schema) {
...
export const coreOperationsToolConfigs = {
  ...
  'create-note': {
    name: 'create-note',
    handler: async (params: unknown): Promise<any> => {
      try {
          'create-note',
          params
        );
    ...
    formatResult: (note: unknown): string => {
      if (!note) {
  ...
  'list-notes': {
    ...
    formatResult: (notes: unknown[]): string => {
