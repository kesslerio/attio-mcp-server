...
export const batchSearchConfig: UniversalToolConfig = {
  ...
  formatResult: (
    ...
    if (successful.length > 0) {
      ...
      successful.forEach((searchResult: UniversalBatchSearchResult, index: number) => {
        ...
        if (Array.isArray(records) && records.length > 0) {
          ...
          records.slice(0, displayCount).forEach((record: AttioRecord, recordIndex: number) => {
    ...
    if (failed.length > 0) {
      summary += `\nFailed searches:\n`;
      failed.forEach((searchResult: UniversalBatchSearchResult, index: number) => {
