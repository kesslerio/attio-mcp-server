...
export const formatterConfigs = {
  details: {
    ...
    formatResult: (company: Company) => {
      ...
      const description =
        company.values?.description?.[0]?.value || 'No description available';
      ...
      const employeeRange =
        company.values?.employee_range?.[0]?.option?.title || 'Not available';
      const foundationDate =
        company.values?.foundation_date?.[0]?.value || 'Not available';
...
For full details, use get-company-json with this ID: ${companyId}`;
  ...
  basicInfo: {
    ...
    formatResult: (company: Partial<Company>) => {
      ...
      const typePersona =
        company.values?.type_persona?.[0]?.option?.title || 'Not available';
      const employees =
        company.values?.employee_range?.[0]?.option?.title || 'Not available';
      const founded =
        company.values?.foundation_date?.[0]?.value || 'Not available';
...
${description}`;
  ...
  contactInfo: {
    ...
    formatResult: (company: Partial<Company>) => {
      ...
      const website = company.values?.website?.[0]?.value || 'Not available';
      const phone =
        company.values?.company_phone_5?.[0]?.phone_number || 'Not available';
      const location = company.values?.primary_location?.[0];
...
${address || 'Not available'}`;
  ...
  businessInfo: {
    ...
    formatResult: (company: Partial<Company>) => {
      ...
      const categories =
        company.values?.categories?.map((cat: any) => cat.option?.title) || [];
      const industry = company.values?.industry?.[0]?.value || 'Not available';
      const revenue =
        company.values?.estimated_arr_usd?.[0]?.option?.title ||
        'Not available';
      const funding =
        company.values?.funding_raised_usd?.[0]?.value || 'Not available';
      const employees =
        company.values?.employee_range?.[0]?.option?.title || 'Not available';
      const founded =
        company.values?.foundation_date?.[0]?.value || 'Not available';
...
${services.length > 0 ? JSON.stringify(services, null, 2) : 'None'}`;
  ...
  socialInfo: {
    ...
    formatResult: (company: Partial<Company>) => {
      ...
      const instagram =
        company.values?.instagram?.[0]?.value || 'Not available';
      const angellist =
        company.values?.angellist?.[0]?.value || 'Not available';
      const twitterFollowers =
        company.values?.twitter_follower_count?.[0]?.value || 'Not available';
