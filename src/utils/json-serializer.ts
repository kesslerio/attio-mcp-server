 ...
 */
export function safeJsonStringify(obj: any, options: SerializationOptions = {}): string {
 ...
 */
export function hasCircularReferences(obj: any, maxDepth: number = 10): boolean {
  const seen = new WeakSet();
 ...
 */
export function createSafeCopy(obj: any, options: SerializationOptions = {}): any {
  const jsonString = safeJsonStringify(obj, options);
  const validation = validateJsonString(jsonString);
  
  if (!validation.isValid) {
    console.error('[createSafeCopy] Failed to create safe copy:', validation.error);
    return {
