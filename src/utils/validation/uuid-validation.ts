...
export function validateUUIDWithDetails(uuid: string, _options?: { resourceType?: string; operation?: string }): UUIDValidationResult {
...
export function createInvalidUUIDError(
  ...
  const validation = validateUUIDWithDetails(recordId, { resourceType, operation });
  ...
  return new EnhancedApiError(
    ...
    operation,
    {
      field: 'record_id',
      resourceType,
      documentationHint: validation.suggestion || 'Expected UUID format (e.g., "a1b2c3d4-e5f6-7890-abcd-ef1234567890")'
    }
  );
}
