...
export const ERROR_MESSAGES = {
  ...
  FILTERS_NOT_ARRAY: (type: string) => `Invalid filter structure: 'filters' property must be an array but got ${type}`,
  ...
  INVALID_FILTER_STRUCTURE: (index: number, reason: string) => `Invalid filter structure at index ${index}: ${reason}`,
 ...
 */
export function getFilterExample(filterType: 'simple' | 'or' | 'multiple' = 'simple'): string {
  switch (filterType) {
    case 'or':
