/**
 * Modular Company Validator - Main implementation
 * 
 * This is the main CompanyValidator class, refactored into a modular structure
 */
...
import { isBooleanFieldByName, inferFieldType, getFieldTypeWithCache } from './field_detector.js';
...
export class CompanyValidator {
  ...
  private static async processFieldValue(
    ...
    try {
      ...
      const fieldType = await getFieldTypeWithCache(fieldName);
      ...
      if (
        ...
        try {
          ...
          console.warn(
            `Failed to convert potential boolean value for field '${fieldName}':`,
  ...
  private static async processAttributeValues(
    ...
    for (const [field, value] of Object.entries(attributes)) {
      if (value !== undefined && value !== null) {
        ...
        processedAttributes[field] = await CompanyValidator.processFieldValue(
          field,
          value
        );
      } else {
  ...
  static extractDomainFromWebsite(attributes: Record<string, CompanyFieldValue>): Record<string, CompanyFieldValue> {
    ...
    if (attributes.website && typeof attributes.website === 'string' && !attributes.domains) {
  ...
  static async validateAttributeUpdate(
    ...
    const processedValue = await CompanyValidator.processFieldValue(
      attributeName,
      attributeValue
    );
  ...
  private static async validateFieldType(
    ...
    const expectedType = await getFieldTypeWithCache(field);
  ...
  private static async getValidatorType(
    ...
    try {
      ...
      const inferredType = inferFieldType(attributeName);
