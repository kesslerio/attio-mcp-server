...
export class EnhancedApiError extends AttioApiError {
  ...
  getContextualMessage(): string {
    ...
    // Issue #416: Fix "Not Found" vs "Invalid Format" confusion
    if (this.context?.httpStatus === 404 && this.context?.recordId) {
      msg = `Record not found: No ${this.context.resourceType} with ID '${this.context.recordId}' exists.`;
    ...
    // General documentation hints
    if (this.context?.documentationHint && !this.context.recordId) { // Avoid duplicating for 404 errors
      msg += ` ${this.context.documentationHint}`;
    }
...
export const ErrorTemplates = {
   ...
   */
  INVALID_SELECT_OPTION: (field: string, value: string, validOptions: string[], resourceType?: string) =>
   ...
   */
  TASK_FIELD_MAPPING: (originalField: string, correctField: string, value?: string) =>
    createEnhancedApiError(
   ...
   */
  GENERIC: (message: string, statusCode: number, endpoint: string, method: string, context?: Partial<ErrorContext>) =>
...
export class ErrorEnhancer {
  ...
  static autoEnhance(
    ...
    if (message.includes('not found') && recordId && this.isValidUUID(recordId)) {
      return ErrorTemplates.RECORD_NOT_FOUND(recordId, resourceType || 'unknown');
    }
    
    if ((message.includes('invalid') || message.includes('format')) && recordId && !this.isValidUUID(recordId)) {
