...
import { OpenAIFetchResult, SupportedAttioType } from './types.js';
...
export async function fetch(id: string): Promise<OpenAIFetchResult> {
  ...
  try {
    ...
    if (result.toolResult?.type === 'text') {
      ...
      const record = parseDetailResponse(result.toolResult.content, resourceType, recordId);
      ...
      if (detailsResult.toolResult?.type === 'text') {
        const additionalDetails = parseDetailResponse(detailsResult.toolResult.content, resourceType, recordId);
    ...
    debug('OpenAI', 'Falling back to direct API call', { resourceType, recordId }, 'fetch');
 ...
 */
function parseRecordId(id: string): { resourceType: SupportedAttioType; recordId: string } {
 ...
 */
function parseDetailResponse(content: string, resourceType: string, recordId: string): any {
  const record: any = {
 ...
 */
async function fetchDirect(resourceType: SupportedAttioType, recordId: string): Promise<OpenAIFetchResult> {
