...
import { OpenAISearchResult, OpenAIFetchResult } from '../types.js';
import { extractAttributeValue, buildTextDescription, generateRecordUrl } from './index.js';
...
export const transformPerson = {
  ...
  toSearchResult(person: any): OpenAISearchResult {
    ...
    const email = extractAttributeValue(person.attributes?.email_addresses || person.email_addresses);
    const title = extractAttributeValue(person.attributes?.job_title || person.job_title);
    const company = extractAttributeValue(person.attributes?.company || person.company);
  ...
  toFetchResult(person: any): OpenAIFetchResult {
    ...
    const professionalFields = [
      ...
      'years_in_role', 'previous_companies'
    ];
    ...
    if (attributes.email_addresses) {
      metadata.email_addresses = extractAttributeValue(attributes.email_addresses);
    }
    ...
    // Lead/contact status
    const statusFields = [
      'lead_status', 'contact_owner', 'last_contacted',
      'next_follow_up'
    ];
