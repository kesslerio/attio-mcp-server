...
export const transformTask = {
  ...
  toSearchResult(task: any): OpenAISearchResult {
    ...
    const content = extractAttributeValue(task.content || task.attributes?.content);
    const status = extractAttributeValue(task.status || task.attributes?.status);
    const assignee = extractAttributeValue(task.assignee?.name || task.attributes?.assignee);
    ...
    if (task.linked_records && task.linked_records.length > 0) {
      ...
      textParts.push(`${linkedCount} linked record${linkedCount > 1 ? 's' : ''}`);
    }
