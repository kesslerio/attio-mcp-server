...
import { OpenAISearchResult, SupportedAttioType } from './types.js';
...
export async function search(query: string): Promise<OpenAISearchResult[]> {
  ...
  const resourceTypes: SupportedAttioType[] = ['companies', 'people', 'lists', 'tasks'];
  ...
  const searchPromises = resourceTypes.map(async (resourceType) => {
    try {
      ...
      debug('OpenAI', `Search result structure for ${resourceType}`, { 
  ...
  debug('OpenAI', `Found ${finalResults.length} results`, { count: finalResults.length }, 'search');
  return finalResults;
}
 ...
 */
export async function searchDirect(query: string): Promise<OpenAISearchResult[]> {
  const client = getAttioClient();
  const results: OpenAISearchResult[] = [];
  
  try {
    // Search companies
    const companiesResponse = await client.post('/objects/companies/records/query', {
      filter: {
        $or: [
