...
export function createAttioApiClient(baseURL: string, apiKey: string): AxiosInstance {
  ...
  instance.interceptors.response.use(
    ...
    async (error: AxiosError | any) => {
      ...
      console.error('[Interceptor] Raw error received by interceptor. Message:', error?.message);
      ...
      if (axios.isAxiosError(error)) {
        ...
        console.error('[Interceptor] Axios error response status:', axiosError.response?.status);
        console.error('[Interceptor] Axios error response headers:', JSON.stringify(axiosError.response?.headers));
        console.error('[Interceptor] Axios error response data (raw):', JSON.stringify(axiosError.response?.data));
        
        if (axiosError.response) {
          console.error(`[Interceptor] API Error: ${axiosError.response.status} ${axiosError.config?.method?.toUpperCase()} ${axiosError.config?.url}`);
          ...
          if (config.retryCount < MAX_RETRIES && (status === 429 || status >= 500)) {
            config.retryCount++;
            console.warn(`[Interceptor] Retrying request (${config.retryCount}/${MAX_RETRIES}) for ${config.url} due to ${status}`);
            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS * (config.retryCount || 1)));
            return instance.request(config);
          }
          return Promise.reject(enhancedError); // Reject with the potentially enhanced error
        }
      } else {
        console.error('[Interceptor] Non-Axios error structure:', JSON.stringify(error, Object.getOwnPropertyNames(error || {})));
      }
