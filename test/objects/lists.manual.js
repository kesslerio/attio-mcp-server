...
async function runTests() {
  try {
    ...
    if (lists.length === 0) {
      console.log(
        'No lists found in the workspace. Some tests will be skipped.'
      );
      return;
    }
    ...
    lists.forEach((list, index) => {
      console.log(
        `${index + 1}. ${list.title || 'Untitled'} (ID: ${
          typeof list.id === 'object' ? JSON.stringify(list.id) : list.id
        }, Object: ${list.object?.slug || 'unknown'}, Entries: ${
    ...
    console.log(
      '\nFirst list object structure:',
      JSON.stringify(lists[0], null, 2)
    );

    // Select the first list for further testing
    const testList = lists[0];
    const listId =
      typeof testList.id === 'object' && testList.id !== null
        ? testList.id.list_id ||
          testList.id.id ||
