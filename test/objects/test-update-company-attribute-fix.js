...
async function testUpdateCompanyAttribute() {
  ...
  const result4 = await runTestCase(
    ...
    (result) => result.isError && result.content[0].text.includes('companyId parameter is required')
  );
  ...
  const result5 = await runTestCase(
    ...
    (result) => result.isError && result.content[0].text.includes('attributeName')
  );
  ...
  const result7 = await runTestCase(
    ...
    (result) => result.isError && result.content[0].text.includes('non-empty string')
  );
...
// Run the test
testUpdateCompanyAttribute().then(success => {
  if (!success) {
    process.exit(1);
  }
}).catch(err => {
