 ...
 */
import { updateCompanyAttribute, updateCompany } from '../dist/objects/companies.js';
...
async function testUpdateCompanyAttributeNull() {
  ...
  try {
    ...
    console.log('Test 1: Using updateCompanyAttribute directly with null value...');
    try {
      const result1 = await updateCompanyAttribute(TEST_COMPANY_ID, ATTRIBUTE_NAME, null);
      console.log('✅ SUCCESS: updateCompanyAttribute accepted null value');
      console.log('Result:', JSON.stringify(result1.values[ATTRIBUTE_NAME], null, 2));
    } catch (error) {
    ...
    console.log('Test 2: Using updateCompany with attributes object containing null...');
    try {
      const result2 = await updateCompany(TEST_COMPANY_ID, {
        [ATTRIBUTE_NAME]: null
      });
      console.log('✅ SUCCESS: updateCompany accepted null value');
      console.log('Result:', JSON.stringify(result2.values[ATTRIBUTE_NAME], null, 2));
    } catch (error) {
    ...
    try {
      ...
      await updateCompanyAttribute(TEST_COMPANY_ID, ATTRIBUTE_NAME, 'test value');
      console.log('Value set successfully');
      
      // Then clear with null
      console.log('Clearing with null...');
      const clearResult = await updateCompanyAttribute(TEST_COMPANY_ID, ATTRIBUTE_NAME, null);
      console.log('✅ SUCCESS: Cleared attribute with null');
      console.log('Result:', JSON.stringify(clearResult.values[ATTRIBUTE_NAME], null, 2));
    } catch (error) {
    ...
    try {
      ...
      const response = await api.patch(`/objects/companies/records/${TEST_COMPANY_ID}`, {
        data: {
          values: {
            [ATTRIBUTE_NAME]: null
          }
      ...
      console.log('Response:', JSON.stringify(response.data.data.values[ATTRIBUTE_NAME], null, 2));
    } catch (error) {
