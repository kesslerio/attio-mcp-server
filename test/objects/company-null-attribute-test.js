...
async function testCompanyNullAttribute() {
  try {
    ...
    if (searchResponse.data.data.length === 0) {
      ...
      const bodyContouringValue = verifyResponse.data.data.values.body_contouring;
      console.log(`\nVerification - body_contouring value: ${JSON.stringify(bodyContouringValue)}`);
      
      if (bodyContouringValue === null || bodyContouringValue === undefined || 
      ...
      console.log(`Using existing company: ${company.values.name[0].value} (ID: ${companyId})\n`);
