...
vi.mock('../../src/api/operations/index', () => ({
  ...
  batchCreateRecords: (vi.fn() as any).mockRejectedValue(
    new Error('Batch API not available')
  ),
  batchUpdateRecords: (vi.fn() as any).mockRejectedValue(
    new Error('Batch API not available')
  ),
}));
...
describe('Batch Company Operations', () => {
  ...
  describe('batchCreateCompanies', () => {
    it('should create multiple companies successfully', async () => {
      ...
      (
        companies.createCompany as vi.MockedFunction<
          typeof companies.createCompany
        >
      )
  ...
  describe('batchUpdateCompanies', () => {
    it('should update multiple companies successfully', async () => {
      ...
      (
        companies.updateCompany as vi.MockedFunction<
          typeof companies.updateCompany
        >
      )
  ...
  describe('batchDeleteCompanies', () => {
    it('should delete multiple companies successfully', async () => {
      (
        companies.deleteCompany as vi.MockedFunction<
          typeof companies.deleteCompany
        >
  ...
  describe('batchSearchCompanies', () => {
    it('should perform multiple searches successfully', async () => {
      ...
      const mockResults2 = [
        {
          id: { record_id: '2' },
          values: { name: [{ value: 'Finance Inc' }] },
        },
        {
          id: { record_id: '3' },
          values: { name: [{ value: 'Finance Ltd' }] },
        },
      ];

      (
        companies.searchCompanies as vi.MockedFunction<
          typeof companies.searchCompanies
        >
      )
  ...
  describe('batchGetCompanyDetails', () => {
    it('should fetch details for multiple companies', async () => {
      ...
      (
        companies.getCompanyDetails as vi.MockedFunction<
          typeof companies.getCompanyDetails
        >
      )
        .mockResolvedValueOnce(mockCompany1)
        .mockResolvedValueOnce(mockCompany2);
  ...
  describe('Issue #153 Validation', () => {
    it('should correctly handle the exact example request from issue #153', async () => {
      ...
      const mockCompanies = [
        ...
        },
        {
          ...
          values: {
            ...
            description: [
              { value: 'Another test company for batch operations' },
            ],
        ...
        },
        {
          ...
          values: {
            ...
            description: [
              { value: 'A third test company for batch operations' },
            ],
      ...
      expect(result.results).toHaveLength(3);
      expect(result.results[0].success).toBe(true);
      expect(result.results[0].data?.values?.name?.[0]?.value).toBe(
        'Test Company Alpha'
      );
      expect(result.results[1].data?.values?.name?.[0]?.value).toBe(
        'Test Company Beta'
      );
      expect(result.results[2].data?.values?.name?.[0]?.value).toBe(
        'Test Company Gamma'
      );
    });
    ...
    describe('Edge Cases', () => {
      it('should reject null params object', async () => {
        await expect(batchCreateCompanies(null as any)).rejects.toThrow(
          'Invalid request: params object is required'
        );
      });

      it('should reject undefined companies array', async () => {
        await expect(batchCreateCompanies({} as any)).rejects.toThrow(
          "Invalid request: 'companies' parameter is required"
        );
      });

      it('should reject non-array companies parameter', async () => {
        await expect(
          batchCreateCompanies({ companies: 'not an array' as any })
        ).rejects.toThrow(
          "Invalid request: 'companies' parameter must be an array"
        );
      });

      it('should reject empty companies array', async () => {
        await expect(batchCreateCompanies({ companies: [] })).rejects.toThrow(
          "Invalid request: 'companies' array cannot be empty"
        );
      });

      it('should reject companies array with null items', async () => {
        await expect(
          batchCreateCompanies({ companies: [null] as any })
        ).rejects.toThrow(
          'Invalid company data at index 0: must be a non-null object'
        );
      });
      ...
      it('should reject companies array with items missing name', async () => {
        await expect(
          ...
          "Invalid company data at index 0: 'name' is required"
        );
      });

      // Similar tests for batch update
      it('should reject null params object for update', async () => {
        await expect(batchUpdateCompanies(null as any)).rejects.toThrow(
          'Invalid request: params object is required'
        );
      });

      it('should reject undefined updates array', async () => {
        await expect(batchUpdateCompanies({} as any)).rejects.toThrow(
          "Invalid request: 'updates' parameter is required"
        );
      });
      ...
      it('should reject companies array with items missing required properties', async () => {
        ...
        await expect(
          ...
          "Invalid update data at index 0: 'attributes' must be a non-null object"
        );
      });
    });
  });
