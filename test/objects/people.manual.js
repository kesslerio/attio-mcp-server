...
async function testPeopleAPIs() {
  try {
    ...
    if (searchResults.length > 0) {
      console.log(
        `Example person: ${JSON.stringify(
          searchResults[0].values?.name?.[0]?.value || 'Unknown'
        )}`
      );
      ...
      if (personId) {
        ...
        console.log(
          `Retrieved details successfully: ${personDetails ? 'Yes' : 'No'}`
        );
