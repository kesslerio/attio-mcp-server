...
async function testNullValueHandling() {
  ...
  try {
    ...
    if (!process.env.ATTIO_API_KEY_REAL) {
      console.log('Please set ATTIO_API_KEY_REAL environment variable to run this test');
      return;
    }
...
async function demonstrateFix() {
  ...
  console.log('The fix in formatAllAttributes now properly handles null values:');
  console.log('');
  ...
  console.log('  } else {');
  console.log('    formatted[key] = await formatAttributeValue(objectSlug, key, value);');
  console.log('  }');
  console.log('}');
  console.log('');
  console.log('This ensures null values are preserved and sent to the API,');
  console.log('preventing the "Cannot convert undefined or null to object" error.');
  console.log('');
  ...
  console.log('Input:', JSON.stringify(mockAttributes, null, 2));
  console.log('Output now: { body_contouring: null, name: {...}, services: {...} }');
