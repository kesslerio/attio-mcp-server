 ...
 */
import { updateCompanyAttribute, getCompanyAttributes } from '../dist/objects/companies.js';
...
async function testClaudeDesktopScenario() {
  ...
  try {
    ...
    const hasBodyContouring = attributesResult.attributes.includes('body_contouring');
    console.log(`Has body_contouring attribute: ${hasBodyContouring}`);
    
    if (hasBodyContouring) {
      const bodyContouringValue = await getCompanyAttributes(TEST_COMPANY_ID, 'body_contouring');
      console.log(`Current body_contouring value: ${JSON.stringify(bodyContouringValue.value)}`);
    }
    
    console.log('\n---\n');
    
    // Step 2: Attempt to update body_contouring with null
    console.log('Step 2: Attempting to update body_contouring with null (Claude Desktop scenario)...');
    console.log('Request:');
    ...
    try {
      const result = await updateCompanyAttribute(TEST_COMPANY_ID, 'body_contouring', null);
      console.log('Response: Company updated:', result.values?.name?.[0]?.value || 'Unknown');
      console.log(`ID: ${result.id?.record_id}`);
      
      // Check the value after update
      const afterValue = await getCompanyAttributes(TEST_COMPANY_ID, 'body_contouring');
      console.log(`\nVerification - body_contouring after update: ${JSON.stringify(afterValue.value)}`);
    ...
    try {
      const workaroundResult = await updateCompany(TEST_COMPANY_ID, {
        body_contouring: 'coolsculpting, coolsculpting elite, liposuction, KYBELLA, tummy tuck (abdominoplasty), body sculpting/body shaping'
      });
      console.log('Company updated:', workaroundResult.values?.name?.[0]?.value || 'Unknown');
