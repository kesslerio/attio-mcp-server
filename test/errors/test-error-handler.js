...
function formatErrorResponse(error, type = ErrorType.UNKNOWN_ERROR, details) {
  ...
  const errorMessage = normalizedError.message === undefined || normalizedError.message === null
...
function createErrorResult(error, url, method, responseData = {}) {
  ...
  console.log(`[Test] Processing error for ${method} ${url}:`, normalizedError.message);
  ...
  if (message && (message.includes('network') || message.includes('connection'))) {
...
function testCircularReferenceError() {
  ...
  try {
    ...
    if (result.error.message) {
      ...
      if (result.error.details && result.error.details.rawError.includes('Circular')) {
        console.log('  - Circular reference was properly handled in details');
      }
    } else {
      console.log('❌ Failed: Did not properly handle error with circular reference');
    }
  } catch (e) {
    console.error('❌ Test failed with exception:', e);
  }
