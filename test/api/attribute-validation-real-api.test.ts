...
const SKIP_TESTS = process.env.SKIP_INTEGRATION_TESTS === 'true' || !process.env.ATTIO_API_KEY;
...
describe(`Attribute Validation with Real Attio API (${SKIP_TESTS ? 'SKIPPED' : 'ACTIVE'})`, () => {
  // Only run tests if not skipped
  beforeAll(() => {
    // Skip all tests if SKIP_INTEGRATION_TESTS is true
    if (SKIP_TESTS) {
      console.log('Skipping API integration tests - no API key or SKIP_INTEGRATION_TESTS=true');
      return;
    }
  ...
  describe(`Company validation with real attributes (${TEST_PREFIX})`, () => {
    ...
    it[TEST_PREFIX]('should fetch real attribute metadata from Attio API', async () => {
    ...
    it[TEST_PREFIX]('should validate company creation with type conversion', async () => {
    ...
    it[TEST_PREFIX]('should validate company update with type conversion', async () => {
    ...
    it[TEST_PREFIX]('should reject invalid attribute values', async () => {
      ...
      await expect(CompanyValidator.validateCreate(invalidData))
        .rejects
        .toThrow();
    });
  });
