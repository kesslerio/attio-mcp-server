...
globalThis.fetch = async (url, options) => {
  ...
  if (options?.body) {
    ...
    if (bodyStr.includes('b2b_segment')) {
      console.error('[FETCH] ERROR: Untranslated attribute "b2b_segment" in request body!');
    }
  }
...
try {
  ...
  axios.post = function(url, data, config) {
    ...
    if (JSON.stringify(data).includes('b2b_segment')) {
      console.error('[AXIOS] ERROR: Untranslated attribute "b2b_segment" in request!');
...
async function testCompleteFlow() {
  try {
    ...
    // Step 1: Translate
    console.log('\n--- Step 1: Translation ---');
    const translated = translateAttributeNamesInFilters(originalFilter, ResourceType.COMPANIES);
    ...
    console.log('\n--- Step 3: API Call ---');
    try {
      const results = await advancedSearchObject(ResourceType.COMPANIES, translated);
