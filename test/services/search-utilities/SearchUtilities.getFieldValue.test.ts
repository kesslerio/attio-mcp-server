...
describe('SearchUtilities.getFieldValue', () => {
  ...
  it('returns first item for array of strings', () => {
    const rec = { values: { tags: ['alpha', 'beta'] } } as unknown as AttioRecord;
    expect(SearchUtilities.getFieldValue(rec, 'tags')).toBe('alpha');
  });

  it('returns first .value for array of objects', () => {
    const rec = { values: { email_addresses: [{ value: 'a@example.com' }] } } as unknown as AttioRecord;
    expect(SearchUtilities.getFieldValue(rec, 'email_addresses')).toBe('a@example.com');
  });

  it('returns .value for object with value', () => {
    const rec = { values: { description: { value: 'About us' } } } as unknown as AttioRecord;
