...
export class E2EAssertions {
  ...
  static expectPersonRecord(person: TestDataObject): void {
    ...
    if (person.values.name) {
      ...
      if (nameEntry && typeof nameEntry === 'object' && 'attribute_type' in nameEntry) {
        // New API structure: personal-name attribute type
        expect(nameEntry.attribute_type, 'Person name should have personal-name type').toBe('personal-name');
        expect(
