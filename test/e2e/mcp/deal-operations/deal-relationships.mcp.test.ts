...
describe('TC-D08 to TC-D10: Deal Relationship Operations', () => {
  ...
  afterAll(async () => {
    ...
    if (passRate < 100) {
      console.warn(
        `⚠️  P1 Quality Gate Warning: Deal Relationships pass rate ${passRate.toFixed(1)}% below 100% threshold`
      );
    }
  });
  ...
  it('TC-D08: should associate deal with company', async () => {
    ...
    try {
      ...
      testCompanyId = QAAssertions.assertRecordCreated(
        companyResult,
        'companies'
      );
      testCase.trackRecord('companies', testCompanyId);
  ...
  it('TC-D09: should associate deal with people/contacts', async () => {
    ...
    try {
      ...
      testCase.trackRecord('people', testPersonId);
  ...
  it('TC-D10: should search deals by associated records', async () => {
    ...
    try {
      ...
      const searchDealId = QAAssertions.assertRecordCreated(
        dealResult,
        'deals'
      );
      testCase.trackRecord('deals', searchDealId);
