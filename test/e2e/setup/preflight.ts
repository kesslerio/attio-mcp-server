...
export function validateApiKey(): PreflightResult {
  ...
  if (!apiKey) {
    errors.push({
      ...
      suggestion: 'Set the ATTIO_API_KEY environment variable:\n' +
  ...
  if (apiKey.includes(' ') && !apiKey.startsWith('Bearer ')) {
    warnings.push({
      ...
      suggestion: 'Check if you accidentally included extra spaces when copying the key'
    });
  }
...
export function validateTestEnvironment(): PreflightResult {
  ...
  if (missingTestConfig.length > 0) {
    warnings.push({
      ...
      suggestion: 'Run "npm run setup:test-data" to create test fixtures, or tests will create temporary data'
    });
  }
  ...
  if (process.env.CI === 'true') {
    ...
    if (!apiKeyResult.valid) {
      errors.push({
        ...
        message: 'Running in CI environment but ATTIO_API_KEY is not properly configured',
        suggestion: 'Configure ATTIO_API_KEY as a secret in your CI environment',
        fatal: true
      });
    }
  }
...
export function runPreflightChecks(): void {
  ...
  if (fatalErrors.length > 0) {
    ...
    console.error('  1. Obtain an API key from Attio (https://app.attio.com/settings/api-keys)');
  ...
  if (allWarnings.length > 0) {
    console.log(`  ℹ️  ${allWarnings.length} warning(s) - tests will continue\n`);
  }
}
