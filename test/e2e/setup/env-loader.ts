...
if (existsSync(mainEnvPath)) {
  const result = dotenv.config({ path: mainEnvPath, debug: false });
  if (result.parsed) {
    console.error(
...
if (existsSync(e2eEnvPath)) {
  const result = dotenv.config({ path: e2eEnvPath, debug: false });
  if (result.parsed) {
    console.error(
...
if (existsSync(localEnvPath)) {
  const result = dotenv.config({ path: localEnvPath, debug: false });
  if (result.parsed) {
    console.error(
...
if (missingVars.length > 0) {
  ...
  console.error('[E2E Setup] Tests will be skipped unless these are set.');
} else {
