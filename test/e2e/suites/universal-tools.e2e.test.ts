...
async function createTestRecord(
  ...
  if (createResponse.isError) {
    throw new Error(
      `Failed to create test ${resourceType.slice(0, -1)}: ${createResponse.error?.message || 'Unknown error'}`
    );
  }
  ...
  if (!idMatch) {
    throw new Error(
      `Could not extract record ID from response: ${responseText}`
    );
  }
