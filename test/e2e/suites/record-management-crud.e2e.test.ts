...
import { callUniversalTool, validateTestEnvironment, getToolMigrationStats } from '../utils/enhanced-tool-caller.js';
...
describe.skipIf(!process.env.ATTIO_API_KEY || process.env.SKIP_E2E_TESTS === 'true')('Record Management E2E – CRUD', () => {
  ...
  afterAll(async () => { endTestSuite(); console.error('✅ Record Management E2E – CRUD completed'); }, 60000);
  beforeEach(() => { vi.clearAllMocks(); });

  it('creates company and person records', async () => {
    const companyResponse = asToolResponse(await callUniversalTool('create-record', { resource_type: 'companies', record_data: CompanyFactory.create() as any }));
    E2EAssertions.expectMcpSuccess(companyResponse);
    const company = E2EAssertions.expectMcpData(companyResponse)!; E2EAssertions.expectCompanyRecord(company); testCompaniesRecord.push(company);

    const personResponse = asToolResponse(await callUniversalTool('create-record', { resource_type: 'people', record_data: PersonFactory.create() as any }));
    E2EAssertions.expectMcpSuccess(personResponse);
    const person = E2EAssertions.expectMcpData(personResponse)!; E2EAssertions.expectPersonRecord(person); testPeopleRecord.push(person);
  }, 60000);
});
