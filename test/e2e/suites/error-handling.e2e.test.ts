...
describe.skipIf(!process.env.ATTIO_API_KEY || process.env.SKIP_E2E_TESTS === 'true')('Error Handling E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (!companyResult.isError && companyResult.content?.[0]?.data?.id?.record_id) {
      ...
      if (!personResult.isError && personResult.content?.[0]?.data?.id?.record_id) {
  ...
  describe('Invalid Parameters and Validation Errors', () => {
    ...
    it('should handle invalid record IDs gracefully', async () => {
      ...
      E2EAssertions.expectMcpError(response);
      expect(response.error).toMatch(/(not found|invalid|does not exist|validation|parameter error)/i);
    });
  ...
  describe('Resource Not Found Scenarios', () => {
    it('should handle company not found errors', async () => {
      ...
      E2EAssertions.expectMcpError(response);
      expect(response.error).toMatch(/(not found|does not exist|invalid|validation|parameter error)/i);
    });
    ...
    it('should handle task not found errors', async () => {
      ...
      expect(response.error).toMatch(/(not found|does not exist|invalid|cannot read|undefined|validation|parameter error)/i);
    });
  ...
  describe('Cross-Tool Error Propagation', () => {
    it('should handle errors when linking non-existent records', async () => {
      ...
      if (!taskResponse.isError && taskResponse.content?.[0]?.data?.id?.record_id) {
    ...
    it('should handle cascading delete scenarios', async () => {
      ...
      if (!companyResponse.isError && companyResponse.content?.[0]?.data?.id?.record_id) {
  ...
  describe('API Limits and Rate Handling', () => {
    it('should handle large result sets appropriately', async () => {
      ...
      if (!response.isError && response.content) {
        console.log(`📊 Large result set test returned: ${response.content.length} items`);
      }
    });
  ...
  describe('Edge Case Error Scenarios', () => {
    it('should handle concurrent modifications', async () => {
      if (!testCompanyId) {
        console.log('⏭️  Skipping concurrent modification test - no test company');
        return;
      }
    ...
    it('should handle circular relationship scenarios', async () => {
      if (!testCompanyId || !testPersonId) {
        console.log('⏭️  Skipping circular relationship test - missing test data');
        return;
      }
    ...
    it('should handle malformed JSON in parameters', async () => {
      ...
      const response = await callUniversalTool('create-record', {
        ...
        record_data: {
          ...
          custom_fields: { 
            weird_data: { deeply: { nested: { object: 'with very long chain' } } }
          }
        }
      });
  ...
  describe('Error Recovery and Resilience', () => {
    ...
    it('should maintain data consistency after errors', async () => {
      ...
      if (!createResponse.isError && createResponse.content?.[0]?.data?.id?.record_id) {
