...
testMappings.forEach(({ input, expected }) => {
  ...
  console.log(`"${input}" → "${result}" ${success ? '✅' : `❌ (expected: ${expected})`}`);
});
...
const translatedFilter = translateAttributeNamesInFilters(complexFilter, ResourceType.COMPANIES);
...
console.log('\n=== Summary ===');
console.log(`All mappings working correctly: ${allMapped ? '✅ YES' : '❌ NO'}`);
...
console.log('- User-defined custom mappings (Recent News/Press → recent_news__press)');
