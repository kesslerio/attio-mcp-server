...
describe('uri-parser', () => {
  describe('parseResourceUri', () => {
    ...
    it('should throw for invalid URI formats', () => {
      ...
      expect(() => parseResourceUri('attio://')).toThrow(
        'Invalid resource URI format'
      );
      expect(() => parseResourceUri('attio://invalid/id')).toThrow(
        'Unsupported resource type'
      );
      expect(() => parseResourceUri('http://attio.com/people/123')).toThrow(
        'Invalid resource URI format'
      );
    });

    it('should handle IDs with special characters', () => {
      expect(parseResourceUri('attio://people/user-123_456')).toEqual([
        ResourceType.PEOPLE,
        'user-123_456',
      ]);
      expect(parseResourceUri('attio://companies/acme.corp')).toEqual([
        ResourceType.COMPANIES,
        'acme.corp',
      ]);
    });
    ...
    it('should handle empty IDs but throw appropriate error', () => {
      expect(() => parseResourceUri('attio://people/')).toThrow(
        'Invalid resource URI format'
      );
    });
  });
  ...
  describe('formatResourceUri', () => {
    it('should correctly format URIs', () => {
      expect(formatResourceUri(ResourceType.PEOPLE, 'person123')).toBe(
        'attio://people/person123'
      );
      expect(formatResourceUri(ResourceType.COMPANIES, 'company456')).toBe(
        'attio://companies/company456'
      );
    });

    it('should handle IDs with special characters', () => {
      expect(formatResourceUri(ResourceType.PEOPLE, 'user-123_456')).toBe(
        'attio://people/user-123_456'
      );
      expect(formatResourceUri(ResourceType.COMPANIES, 'acme.corp')).toBe(
        'attio://companies/acme.corp'
      );
    });
    ...
    it('should not validate ID format', () => {
      ...
      expect(formatResourceUri(ResourceType.PEOPLE, '')).toBe(
        'attio://people/'
      );
      expect(formatResourceUri(ResourceType.COMPANIES, ' ')).toBe(
        'attio://companies/ '
      );
    });
  });
