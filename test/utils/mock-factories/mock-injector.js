...
function retrieveMock(id) {
    return mockStorage.get(id);
}
function getAllMocksByType(type) {
    const results = [];
...
export class TaskMockInjector {
     ...
     */
    static async createTask(content, options = {}, realCreateTask) {
        if (shouldUseMockData()) {
            ...
            const mockTask = TaskMockFactory.create({
                ...
                linked_records: options.recordIds ? options.recordIds.map(id => ({
                    id,
     ...
     */
    static async updateTask(taskId, updateData, realUpdateTask) {
        if (shouldUseMockData()) {
            ...
            if (updatedTask.id && typeof updatedTask.id === 'object') {
                updatedTask.id.record_id = taskId;
                updatedTask.id.task_id = taskId;
            }
     ...
     */
    static async getTask(taskId, realGetTask) {
        if (shouldUseMockData()) {
     ...
     */
    static async listTasks(realListTasks) {
        if (shouldUseMockData()) {
     ...
     */
    static async deleteTask(taskId, realDeleteTask) {
        if (shouldUseMockData()) {
...
export class CompanyMockInjector {
    static async createCompany(companyData, realCreateCompany) {
        if (shouldUseMockData()) {
    ...
    static async getCompany(companyId, realGetCompany) {
        if (shouldUseMockData()) {
            ...
            if (mockCompany.id && typeof mockCompany.id === 'object') {
                mockCompany.id.record_id = companyId;
            }
            return mockCompany;
        }
        if (!realGetCompany) {
...
export class PersonMockInjector {
    static async createPerson(personData, realCreatePerson) {
        if (shouldUseMockData()) {
    ...
    static async getPerson(personId, realGetPerson) {
        if (shouldUseMockData()) {
            ...
            if (mockPerson.id && typeof mockPerson.id === 'object') {
                mockPerson.id.record_id = personId;
            }
            return mockPerson;
        }
        if (!realGetPerson) {
...
export class ListMockInjector {
    static async createList(listData, realCreateList) {
        if (shouldUseMockData()) {
    ...
    static async getList(listId, realGetList) {
        if (shouldUseMockData()) {
    ...
    static async listLists(options = {}, realListLists) {
        if (shouldUseMockData()) {
...
export class UniversalMockInjector {
     ...
     */
    static async inject(resourceType, operation, data = {}, realOperation) {
        if (!shouldUseMockData()) {
            if (!realOperation) {
    ...
    static handleTaskOperation(operation, data) {
        switch (operation) {
            case 'create':
    ...
    static handleCompanyOperation(operation, data) {
        switch (operation) {
            case 'create':
    ...
    static handlePersonOperation(operation, data) {
        switch (operation) {
            case 'create':
    ...
    static handleListOperation(operation, data) {
        switch (operation) {
            case 'create':
...
//# sourceMappingURL=mock-injector.js.map