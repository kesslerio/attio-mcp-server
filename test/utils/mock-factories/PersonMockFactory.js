...
export class PersonMockFactory {
    ...
    static create(overrides = {}) {
        ...
        const fullName = overrides.name
            ? (typeof overrides.name === 'string' ? overrides.name : `${overrides.name.first_name} ${overrides.name.last_name}`)
            : `${firstName} ${lastName}`;
        ...
        Object.entries(overrides).forEach(([key, value]) => {
            if (!['name', 'email_addresses', 'phone_numbers', 'job_title', 'department', 'seniority', 'company', 'created_at', 'updated_at'].includes(key)) {
     ...
     */
    static createExecutive(overrides = {}) {
        const executiveTitles = ['CEO', 'CTO', 'CFO', 'COO', 'VP of Sales', 'VP of Marketing', 'Chief Executive Officer'];
        return this.create({
            ...overrides,
            job_title: overrides.job_title || executiveTitles[Math.floor(Math.random() * executiveTitles.length)],
            seniority: 'Executive'
        });
    }
    /**
     ...
     */
    static createSalesPerson(overrides = {}) {
        const salesTitles = ['Account Executive', 'Sales Representative', 'Business Development Manager', 'Sales Manager'];
        return this.create({
            ...overrides,
            job_title: overrides.job_title || salesTitles[Math.floor(Math.random() * salesTitles.length)],
            department: 'Sales',
     ...
     */
    static createEngineer(overrides = {}) {
        const engineeringTitles = ['Software Engineer', 'Senior Software Engineer', 'Principal Engineer', 'Engineering Manager', 'DevOps Engineer'];
        return this.create({
            ...overrides,
            job_title: overrides.job_title || engineeringTitles[Math.floor(Math.random() * engineeringTitles.length)],
     ...
     */
    static createMarketingPerson(overrides = {}) {
        const marketingTitles = ['Marketing Manager', 'Content Marketing Manager', 'Digital Marketing Specialist', 'Marketing Director'];
        return this.create({
            ...overrides,
            job_title: overrides.job_title || marketingTitles[Math.floor(Math.random() * marketingTitles.length)],
     ...
     */
    static createWithCompany(companyId, overrides = {}) {
        return this.create({
            ...overrides,
            company: companyId
        });
     ...
     */
    static generateEmail(name, suffix) {
    ...
    static getRandomFirstName() {
        const names = [
            ...
            'John', 'Jane', 'Michael', 'Sarah', 'David', 'Lisa', 'Chris', 'Amy',
            'Robert', 'Emily', 'James', 'Jessica', 'William', 'Ashley', 'Daniel', 'Michelle'
        ];
    ...
    static getRandomJobTitle() {
        const titles = [
            ...
            'Data Analyst', 'UX Designer', 'Customer Success Manager', 'Operations Manager',
            'Business Analyst', 'Project Manager', 'Account Executive', 'Content Writer'
        ];
        return titles[Math.floor(Math.random() * titles.length)];
    }
    static getRandomSeniority() {
        const levels = ['Junior', 'Mid-level', 'Senior', 'Lead', 'Principal', 'Executive'];
...
//# sourceMappingURL=PersonMockFactory.js.map