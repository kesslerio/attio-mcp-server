...
describe('Filter Validation Utilities', () => {
  describe('validateFiltersObject', () => {
    ...
    it('should throw error for filters missing filters property', () => {
      expect(() => validateFiltersObject({} as any))
        .toThrow(FilterValidationError);
      
      expect(() => validateFiltersObject({} as any))
        .toThrow(ERROR_MESSAGES.MISSING_FILTERS_PROPERTY);
    });
    ...
    it('should throw error for non-array filters property', () => {
      ...
      expect(() => validateFiltersObject(filters))
        .toThrow(FilterValidationError);
      
      expect(() => validateFiltersObject(filters))
        .toThrow(ERROR_MESSAGES.FILTERS_NOT_ARRAY(typeof filters.filters));
    });
  });
  ...
  describe('collectInvalidFilters', () => {
    ...
    it('should detect missing attribute.slug', () => {
      ...
      const invalidFilters = collectInvalidFilters(filters);
      expect(invalidFilters.length).toBe(1);
      expect(invalidFilters[0].reason).toBe(ERROR_MESSAGES.MISSING_ATTRIBUTE_SLUG);
    });
    ...
    it('should format multiple errors correctly', () => {
      const invalidFilters = [
        ...
        },
        {
          ...
          filter: { attribute: { slug: 'name' }, condition: 'bad', value: 'test' }
        }
      ];
      
      const formatted = formatInvalidFiltersError(invalidFilters);
      expect(formatted).toBe('Filter [0]: missing attribute.slug; Filter [2]: invalid condition');
    });
  });
  ...
  describe('getInvalidFilterReason', () => {
    ...
    it('should detect missing attribute', () => {
      ...
      expect(getInvalidFilterReason(filter)).toBe(ERROR_MESSAGES.MISSING_ATTRIBUTE);
    });
    ...
    it('should detect missing attribute.slug', () => {
      ...
      expect(getInvalidFilterReason(filter)).toBe(ERROR_MESSAGES.MISSING_ATTRIBUTE_SLUG);
    });
    ...
    it('should detect missing condition', () => {
      ...
      expect(getInvalidFilterReason(filter)).toBe(ERROR_MESSAGES.MISSING_CONDITION);
    });
