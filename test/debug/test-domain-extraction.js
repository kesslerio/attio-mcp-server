...
async function testDomainExtraction() {
  ...
  try {
    ...
    const { CompanyValidator } = await import('../../dist/validators/company-validator.js');
    
    // Test cases for domain extraction
    const testCases = [
      {
        name: 'Basic HTTPS URL',
        input: { name: 'Test Company', website: 'https://www.theplasticsdoc.com' },
      ...
      },
      {
        ...
        input: { name: 'Subdomain Company', website: 'https://app.subdomain.com' },
    ...
    for (const testCase of testCases) {
      try {
        const result = CompanyValidator.extractDomainFromWebsite({ ...testCase.input });
        const extractedDomain = result.domains ? result.domains[0] : null;
        
        if (testCase.shouldNotOverwrite) {
          // Should preserve existing domains
          const success = JSON.stringify(result.domains) === JSON.stringify(testCase.input.domains);
          console.log(`  ✅ ${testCase.name}: ${success ? 'PASS' : 'FAIL'} - preserved manual domains`);
        } else if (testCase.expectedDomain === null) {
          // Should not add domains field
          const success = !result.domains;
          console.log(`  ${success ? '✅' : '❌'} ${testCase.name}: ${success ? 'PASS' : 'FAIL'} - no domain extracted`);
        } else {
          // Should extract expected domain
          const success = extractedDomain === testCase.expectedDomain;
          console.log(`  ${success ? '✅' : '❌'} ${testCase.name}: ${success ? 'PASS' : 'FAIL'} - extracted "${extractedDomain}"`);
        }
      } catch (error) {
    ...
    if (validated.domains && validated.domains.includes('theplasticsdoc.com')) {
      console.log('✅ Domain extraction in validation: PASS - theplasticsdoc.com extracted');
    } else {
      console.log('❌ Domain extraction in validation: FAIL - domain not extracted');
    }
    
    // Test 3: Test actual company creation
    console.log('\\n3️⃣ Testing actual company creation...');
    
    const { createCompany } = await import('../../dist/objects/companies/basic.js');
    ...
    if (createdCompany.values?.domains) {
      ...
      if (hasDomain) {
        console.log('✅ End-to-end domain extraction: PASS - domain populated in Attio');
      } else {
        console.log('❌ End-to-end domain extraction: FAIL - domain not found in response');
        console.log('   Expected: test-domain-extraction.com');
        console.log('   Actual domains:', domains.map(d => d?.domain || d?.value || d));
      }
    } else {
      console.log('❌ End-to-end domain extraction: FAIL - no domains field found');
    }
