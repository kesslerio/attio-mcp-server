...
async function testIssue221Scenario() {
  ...
  console.log('Expected: System should auto-extract domain to prevent duplicates');
  console.log('');
  ...
  try {
    ...
    if (companyDetails.values?.domains) {
      ...
      domainFound = domains.some(d => {
        ...
        return domainValue === 'theplasticsdoc.com' || domainValue?.includes('theplasticsdoc.com');
      });
      
      console.log('- Domain extracted:', domainFound ? 'YES ✅' : 'NO ❌');
      console.log('- Domain values found:', domains.map(d => d?.domain || d?.value || d));
    } else {
      ...
      if (duplicateResult.isError) {
        ...
        if (duplicateIdMatch) {
          ...
          if (duplicateId === companyId) {
            console.log('✅ Duplicate prevention: PASS - Same company returned');
          } else {
    ...
    if (domainFound) {
      ...
      console.log('');
      console.log('💡 Check implementation and ensure domain extraction logic is working');
    }
