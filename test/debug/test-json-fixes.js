...
async function testJsonSerializer() {
  ...
  try {
    ...
    console.log('   Has circular refs (normal obj):', hasCircularReferences({ name: 'test' }));
    
    // Test safe JSON stringify
    console.log('\n2. Testing safe JSON stringify:');
    const safeJson = safeJsonStringify(circularObj);
    console.log('   Circular object serialized successfully:', safeJson.length > 0);
    ...
    console.log('   MCP response sanitized successfully:', sanitized.content[0].text === 'Test response');
    ...
    console.log('   Complex data serialized successfully:', complexSerialized.length > 0);
...
async function testFormatters() {
  ...
  try {
    const { formatResponse } = await import('../../dist/handlers/tools/formatters.js');
    ...
    console.log('   Formatted successfully:', result.content[0].text.includes('test'));
    ...
    console.log('   Complex data formatted successfully:', complexResult.content[0].text.length > 0);
...
async function testErrorHandlers() {
  ...
  try {
    const { createErrorResult, formatErrorResponse } = await import('../../dist/utils/error-handler.js');
    ...
    console.log('   Error result created successfully:', errorResult.isError === true);
