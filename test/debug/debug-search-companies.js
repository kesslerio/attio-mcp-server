...
async function testSearchCompaniesFlow() {
  ...
  try {
    ...
    const { findToolConfig } = await import('../../dist/handlers/tools/registry.js');
    ...
    const { extractDomain, extractAllDomains } = await import('../../dist/utils/domain-utils.js');
    ...
    if (!process.env.ATTIO_API_KEY) {
      ...
      console.log('Final result: "Found 0 companies:" (incorrect - should find domain matches)');
    } else {
    ...
    if (mcpResponse.isError) {
      ...
      console.log('This indicates that the MCP server startup process is not being simulated correctly.');
    } else {
      ...
      if (responseText.includes('Found 0 companies:')) {
        ...
        console.log('1. No companies with "theplasticsdoc.com" domain exist in the CRM');
