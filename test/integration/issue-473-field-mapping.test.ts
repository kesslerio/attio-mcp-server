...
describe('Issue #473: Field Mapping Consistency', () => {
  ...
  afterAll(async () => {
    ...
    for (const record of testRecords) {
      try {
        ...
        console.log(`✅ Cleaned up ${record.resource_type} record: ${record.id}`);
      } catch (error) {
        console.warn(`Failed to cleanup ${record.resource_type} record ${record.id}:`, error);
      }
    }
  });
  ...
  describe('Company Field Mapping', () => {
    test('should correctly map and persist company fields', async () => {
      ...
      testRecords.push({ id: companyId, resource_type: UniversalResourceType.COMPANIES });
      ...
      const mappingErrors: Array<{field: string, expected: unknown, actual: unknown}> = [];
      ...
      Object.entries(updateData).forEach(([field, expectedValue]) => {
        ...
        if (field === 'website' && actualValue && Array.isArray(retrieved.values?.domains)) {
          ...
          if (domains.length > 0) {
            ...
            if (expectedValue.toString().includes(domainValue) || domainValue === expectedValue) {
      ...
      if (mappingErrors.length > 0) {
        const errorDetails = mappingErrors.map(e => 
          `${e.field}: expected ${e.expected}, got ${e.actual}`
        ).join('; ');
  ...
  describe('People Field Mapping', () => {
    test('should correctly map and persist people fields', async () => {
      ...
      testRecords.push({ id: personId, resource_type: UniversalResourceType.PEOPLE });
      ...
      Object.entries(updates).forEach(([field, expectedValue]) => {
        ...
        for (const possibleField of possibleFields) {
          ...
          if (fieldData) {
            if (Array.isArray(fieldData) && fieldData.length > 0) {
              actualValue = fieldData[0]?.value || fieldData[0]?.phone_number || fieldData[0];
            } else {
              actualValue = fieldData;
            }
            break;
        ...
        if (actualValue !== expectedValue) {
          errors.push(`Field "${field}" not updated correctly: expected ${expectedValue}, got ${actualValue}`);
  ...
  describe('Special Character Field Mapping', () => {
    test('should handle special characters correctly', async () => {
      ...
      testRecords.push({ id: recordId, resource_type: UniversalResourceType.COMPANIES });
      ...
      Object.entries(specialData).forEach(([field, expected]) => {
        ...
        if (actual !== expected) {
          specialCharErrors.push(`Field "${field}" corrupted: expected "${expected}", got "${actual}"`);
      ...
      if (specialCharErrors.length > 0) {
        throw new Error(`Special character handling errors: ${specialCharErrors.join('; ')}`);
      }
      ...
      Object.entries(updateData).forEach(([field, expected]) => {
        ...
        if (actual !== expected) {
          specialCharErrors.push(`Updated field "${field}" corrupted: expected "${expected}", got "${actual}"`);
      ...
      if (specialCharErrors.length > 0) {
        throw new Error(`Special character update errors: ${specialCharErrors.join('; ')}`);
      }
    ...
    test('should behave consistently across all resource types', async () => {
      ...
      for (const testCase of testCases) {
        try {
          ...
          const recordId = record.id.record_id;
          testRecords.push({ id: recordId, resource_type: testCase.resource_type });
          ...
          if (updateField === 'website' && foundField === 'domains') {
            ...
            success = Array.isArray(fieldData) && fieldData.some(item => 
          ...
          consistencyResults.push({
            resource_type: testCase.resource_type,
            success,
            error: success ? undefined : `Expected ${expectedValue}, got ${actualValue}`
          });
          
          console.log(`${success ? '✅' : '❌'} ${testCase.resource_type} consistency test ${success ? 'passed' : 'failed'}`);
          ...
          console.log(`❌ ${testCase.resource_type} consistency test failed:`, error);
        }
      }
      
      const failedTests = consistencyResults.filter(r => !r.success);
      if (failedTests.length > 0) {
        const errorDetails = failedTests.map(f => `${f.resource_type}: ${f.error}`).join('; ');
