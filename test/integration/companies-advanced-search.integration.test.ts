 ...
 */
import { advancedSearchCompanies, createNameFilter } from '../../src/objects/companies/search';
...
describe('Companies Advanced Search Integration', () => {
  ...
  testMethod('should successfully search companies with valid filters', async () => {
  ...
  testMethod('should throw descriptive error for invalid filter structure', async () => {
    const invalidFilters = {} as any; // Missing filters property
    
    await expect(advancedSearchCompanies(invalidFilters))
      .rejects
    ...
    try {
      ...
      expect((error as FilterValidationError).message).toContain('Advanced company search filter invalid');
      expect((error as FilterValidationError).message).toContain('MISSING_FILTERS_PROPERTY');
    }
  });
  
  // Test invalid search with invalid condition
  testMethod('should throw descriptive error for invalid filter condition', async () => {
    ...
    try {
      ...
      expect((error as FilterValidationError).message).toContain('Advanced company search filter invalid');
      expect((error as FilterValidationError).message).toContain('Invalid condition');
  ...
  testMethod('should throw descriptive error for missing attribute', async () => {
    const invalidFilters = {
      filters: [
        {
    ...
    try {
      ...
      expect(error instanceof FilterValidationError).toBeTruthy();
      expect((error as FilterValidationError).message).toContain('missing attribute');
    }
  });
  
  // Test with OR logic
  testMethod('should successfully search with OR logic', async () => {
    const filters = {
      filters: [
        {
          attribute: { slug: 'name' },
  ...
  testMethod('should handle empty filters array gracefully', async () => {
    const emptyFilters = {
      filters: []
    };
