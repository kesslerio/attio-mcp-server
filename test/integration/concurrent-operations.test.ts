...
describe.skipIf(skipIntegrationTests)('Concurrent Operations - Integration Tests', () => {
  ...
  describe('Concurrent Updates', () => {
    it('should handle concurrent attribute updates', async () => {
      ...
      console.log(`Concurrent updates: ${successful.length} succeeded, ${failed.length} failed`);
      ...
      const hasExpectedValues = updates.some(update => {
        ...
        if (Array.isArray(value) && value.length > 0) {
          return value[0].value === update.value || value[0].option?.title === update.value;
        }
        return false;
      });
  ...
  describe('Concurrent Creates and Deletes', () => {
    it('should handle concurrent company creation', async () => {
      ...
      const createPromises = Array(numCompanies).fill(0).map((_, i) => 
        createCompany({
          name: `${baseName} - ${i}`,
    ...
    it('should handle create and immediate delete race condition', async () => {
      ...
      const [updateResult, deleteResult] = await Promise.allSettled([
        updateCompanyAttribute(company.id.record_id, 'description', 'Updated before delete'),
        deleteCompany(company.id.record_id)
      ]);
      
      // One should succeed, one might fail
      const successCount = [updateResult, deleteResult].filter(r => r.status === 'fulfilled').length;
      expect(successCount).toBeGreaterThan(0);
  ...
  describe('Concurrent Searches and Updates', () => {
    it('should handle searches during updates', async () => {
      ...
      const operations = [
        ...
        updateCompanyAttribute(companies[0].id.record_id, 'description', 'Updated A'),
        searchCompanies(uniquePrefix),
        updateCompanyAttribute(companies[1].id.record_id, 'description', 'Updated B'),
        searchCompanies(uniquePrefix)
      ];
    ...
    it('should handle high volume of concurrent operations', async () => {
      ...
      for (let i = 0; i < operationCount; i++) {
        ...
        switch (operationType) {
          case 0:
            operations.push(
              updateCompanyAttribute(company.id.record_id, 'description', `Update ${i}`)
            );
            break;
          case 1:
            operations.push(
              getCompanyDetails(company.id.record_id)
            );
            break;
          case 2:
            operations.push(
