...
describe('Real API Integration Tests', () => {
  ...
  describe('Companies Module - Real API', () => {
    ...
    it('should search for the created company', async () => {
      ...
      const foundCompany = results.find(c => 
        c.values.name?.[0]?.value === testCompanyName
      );
    ...
    it('should update the company', async () => {
      ...
      expect(updated.values.description?.[0]?.value).toBe('Updated integration test company');
    });
  ...
  describe('People Module - Real API', () => {
    it('should create a real person', async () => {
      ...
      expect(person.values.email_addresses[0].email_address).toBe(testPersonEmail);
    ...
    it('should search for the created person', async () => {
      ...
      const foundPerson = results.find(p => 
        p.values.email_addresses?.some((e: any) => e.email_address === testPersonEmail)
      );
    ...
    it('should get person details', async () => {
      ...
      expect(details.values.email_addresses[0].email_address).toBe(testPersonEmail);
    });
  ...
  describe('Error Handling - Real API', () => {
    it('should handle non-existent company gracefully', async () => {
      await expect(getCompanyDetails('non-existent-id'))
        .rejects
        .toThrow();
    });
    ...
    it('should handle non-existent person gracefully', async () => {
      await expect(getPersonDetails('non-existent-id'))
        .rejects
        .toThrow();
    });
