 ...
 */
import { ResourceType, RelationshipType } from '../../src/types/attio';
...
describe('Relationship Filter Performance Tests', () => {
  ...
  describe('List membership filters', () => {
    it('should measure creation time for simple list filter', () => {
      ...
      const filter = Filters.Relationship.createRecordsByListFilter(
        resourceType,
        listId
      );
    ...
    it('should measure performance with and without cache', () => {
      ...
      const filterNoCache = Filters.Relationship.createRecordsByListFilter(
        resourceType,
        listId
      );
      ...
      const filterCache = Filters.Relationship.createRecordsByListFilter(
        resourceType,
        listId
      );
      ...
      // Log performance
      console.log(
        `List filter creation time (no cache): ${executionTimeNoCache.toFixed(
          3
        )}ms`
      );
      console.log(
        `List filter creation time (with cache): ${executionTimeCache.toFixed(
          3
        )}ms`
      );
      console.log(
        `Cache speedup: ${(executionTimeNoCache / executionTimeCache).toFixed(
          2
        )}x`
      );
  ...
  describe('Nested relationship filters', () => {
    it('should measure creation time for people by company list filter', () => {
      ...
      // Log performance
      console.log(
        `People by company list filter creation time: ${executionTimeMs.toFixed(
          3
        )}ms`
      );
    ...
    it('should measure creation time for companies by people list filter', () => {
      ...
      const startTime = performance.now();
      const filter =
        Filters.Relationship.createCompaniesByPeopleListFilter(listId);
      const endTime = performance.now();
      ...
      // Log performance
      console.log(
        `Companies by people list filter creation time: ${executionTimeMs.toFixed(
          3
        )}ms`
      );
    ...
    it('should measure nested filter performance with and without cache', () => {
      ...
      // Log performance
      console.log(
        `Nested filter creation time (no cache): ${executionTimeNoCache.toFixed(
          3
        )}ms`
      );
      console.log(
        `Nested filter creation time (with cache): ${executionTimeCache.toFixed(
          3
        )}ms`
      );
      console.log(
  ...
  describe('Filter complexity performance', () => {
    it('should compare performance of simple vs nested relationship filters', () => {
      ...
      const simpleFilter = Filters.Relationship.createRecordsByListFilter(
        resourceType,
        listId
      );
      ...
      // Log performance
      console.log(
        `Simple filter creation time: ${executionTimeSimple.toFixed(3)}ms`
      );
      console.log(
        `Nested filter creation time: ${executionTimeNested.toFixed(3)}ms`
      );
      console.log(
        `Complexity ratio: ${(
          executionTimeNested / executionTimeSimple
        ).toFixed(2)}x`
      );
    ...
    it('should measure performance of very complex nested relationship filter', () => {
      ...
      const peopleWorkingAtThoseCompanies =
        Filters.Relationship.createPeopleByCompanyFilter(companiesInList);

      // Create second level relationship filter
      const companiesEmployingThosePeople =
        Filters.Relationship.createCompaniesByPeopleFilter(
      ...
      // Log performance
      console.log(
        `Very complex filter creation time: ${executionTime.toFixed(3)}ms`
      );
  ...
  describe('Cache efficiency', () => {
    it('should test cache hit rate for repeated similar queries', () => {
      ...
      console.log(
        `Cache hit rate: ${((cacheHits / queryCount) * 100).toFixed(1)}%`
      );
