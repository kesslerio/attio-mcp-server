 ...
 */
import { validateAttributeValue, ValidationResult, AttributeType } from '../../src/validators/attribute-validator.js';
  ...
  describe('validateAttributeValue', () => {
    ...
    describe('Object validation', () => {
      ...
      it('should reject non-object values', () => {
        const result = validateAttributeValue('test', 'not an object', 'object');
        expect(result.valid).toBe(false);
        expect(result.error).toBeDefined();
      });
    });
    ...
    describe('Select validation', () => {
      ...
      it('should accept array of strings', () => {
        expect(validateAttributeValue('test', ['option1', 'option2'], 'select')).toEqual({
          valid: true,
    ...
    describe('Record reference validation', () => {
      it('should accept string IDs', () => {
        expect(validateAttributeValue('test', 'rec_123', 'record-reference')).toEqual({
          valid: true,
      ...
      it('should extract record_id from objects', () => {
        expect(validateAttributeValue('test', { record_id: 'rec_123' }, 'record-reference')).toEqual({
          valid: true,
          convertedValue: 'rec_123'
        });
      });
      ...
      it('should extract id from objects as fallback', () => {
        expect(validateAttributeValue('test', { id: 'rec_123' }, 'record-reference')).toEqual({
          valid: true,
          convertedValue: 'rec_123'
        });
      });
      ...
      it('should process arrays of records', () => {
        ...
        expect(validateAttributeValue('test', input, 'record-reference')).toEqual({
          valid: true,
      ...
      it('should reject invalid record references', () => {
        const result = validateAttributeValue('test', { foo: 'bar' }, 'record-reference');
        expect(result.valid).toBe(false);
        expect(result.error).toBeDefined();
      });
    });
