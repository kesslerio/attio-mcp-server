...
async function updateCompany(companyId, data) {
  try {
    console.log(`Updating company ${companyId} with data:`, JSON.stringify(data, null, 2));
...
async function updateAttribute(companyId, attributeName, attributeValue) {
  try {
    console.log(`Updating attribute ${attributeName} for company ${companyId} with value:`, attributeValue);
    ...
    console.log(`Attribute ${attributeName} updated successfully for company ${companyId}`);
    
    return response.data.data;
  } catch (error) {
    console.error(`Failed to update attribute ${attributeName} for company ${companyId}:`, error.message);
    if (error.response?.data) {
      console.error('API response:', JSON.stringify(error.response.data, null, 2));
    }
    throw error;
...
async function runTests() {
  ...
  try {
    ...
    console.log('\nUpdated company details:', JSON.stringify(updatedCompany, null, 2));
    ...
    console.log('\nFinal company details:', JSON.stringify(finalCompany, null, 2));
