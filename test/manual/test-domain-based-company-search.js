...
import { searchCompanies, searchCompaniesByDomain, smartSearchCompanies } from '../../src/objects/companies/index.js';
import { extractDomain, hasDomainIndicators, extractAllDomains } from '../../src/utils/domain-utils.js';
...
async function testDomainUtils() {
  ...
  for (const testCase of testCases) {
    ...
    console.log(`  - Extract all domains: [${extractAllDomains(testCase).join(', ')}]`);
    console.log('');
  }
}
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      results.slice(0, 10).forEach((company, index) => {
        ...
        console.log(`  ${index + 1}. ${name} (${website}) ${containsDomain ? '🎯 DOMAIN MATCH' : ''}`);
      });
    } else {
