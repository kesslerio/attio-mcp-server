  ...
  try {
    ...
    if (directNotes.length > 0) {
      console.log(
        ...
        JSON.stringify(directNotes[0], null, 2)
      );
      console.log('\nNote fields present:');
      Object.keys(directNotes[0]).forEach((key) => {
        console.log(
          `- ${key}: ${typeof directNotes[0][key]} = ${directNotes[0][key]}`
        );
      });
    }
    ...
    if (
      ...
      console.log(
        '✅ Test setup correct (API key needed for actual execution)'
      );
    } else {
...
if (process.env.ATTIO_API_KEY) {
  ...
  console.log(
    '3. Different timestamp field:',
    formatter(notesWithDifferentTimestamp)
  );
}
