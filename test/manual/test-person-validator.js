...
async function testPersonValidator() {
  ...
  try {
    ...
    try {
      const result4 = await PersonValidator.validateCreate({
        company: 'Test Corp',
      });
    ...
    try {
      ...
      console.log(
        '✅ Correctly failed when objectSlug passed as attributes:',
        error.message
      );
      console.log(
        '   This demonstrates the original bug - when createPerson received "people" instead of attributes'
      );
    }
