...
const LIST_ID = process.env.TEST_LIST_ID || '88709359-01f6-478b-ba66-c07347891b6f';
...
async function testUpdateListEntryStage() {
  ...
  console.log(`- Request format: { data: { values: { stage: "Demo Scheduling" } } }\n`);
...
async function runTests() {
  ...
  try {
    ...
    console.log('\n=== Test Results ===');
    console.log(`Update list entry test: ${updateResult ? 'PASSED' : 'FAILED (Expected if no valid entry ID)'}`);
    console.log(`Missing parameters test: ${missingParamsResult ? 'PASSED' : 'FAILED'}`);
    console.log(`Filter list entries test: ${filterResult ? 'PASSED' : 'FAILED'}`);
    console.log(`Get company lists test: ${getListsResult ? 'PASSED' : 'FAILED'}`);
    ...
    if (criticalTests) {
      ...
      console.log('ℹ️  Update list entry test may fail without valid entry ID - this is expected.');
    } else {
