...
// Skip tests if no API key is provided or if integration tests are disabled
...
describe('Advanced Search Companies Fix', () => {
  // Skip all tests if no API key or integration tests disabled
  if (SKIP_TESTS) {
    test.skip('Skipped: Integration test disabled', () => {
      const reason = !process.env.ATTIO_API_KEY 
        ? 'ATTIO_API_KEY is not set' 
        : 'SKIP_INTEGRATION_TESTS=true';
      console.log(`Skipping tests because ${reason}`);
    });
    return;
  }
