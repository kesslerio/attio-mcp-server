...
describe('Lists Tool Configurations', () => {
  ...
  describe('getListEntries tool', () => {
    it('should call getListEntries and format the results', async () => {
      ...
      expect(formattedResult).toContain(
        'Entry ID: entry1, Record ID: record1 (Company: Acme Inc)'
      );
      expect(formattedResult).toContain(
        'Entry ID: entry2, Record ID: record2 (Company: Globex Corp)'
      );
    });
  });
  ...
  describe('filterListEntries tool', () => {
    it('should call getListEntries with filter parameters', async () => {
      ...
      expect(mockedLists.getListEntries).toHaveBeenCalledWith('list1', 10, 0, {
        filters: [
          {
            attribute: {
              slug: 'stage',
