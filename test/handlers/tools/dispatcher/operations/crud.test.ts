...
describe('CRUD Operation Handlers', () => {
    ...
    testCases.forEach(({ resourceType, description }) => {
      describe(`for ${description}`, () => {
        it('should successfully create a record with valid attributes', async () => {
          ...
          const result = await handleCreateOperation(request, toolConfig, resourceType);
          ...
          expect(result).toEqual({
            content: [{ type: 'text', text: `${resourceType.slice(0, -1)} created successfully` }],
            isError: false,
          });
        });
        ...
        it('should use custom formatResult when provided', async () => {
          const mockResult = { id: 'created-id-123', name: 'Test Company' };
          const customFormatter = vi.fn().mockReturnValue('Custom formatted result');
          ...
          expect(mockFormatResponse).toHaveBeenCalledWith('Custom formatted result');
        });
      ...
      describe(`for ${description}`, () => {
        it('should successfully update a record with valid id and attributes', async () => {
          ...
          const request = createMockRequest('update-company', {
            [idParam]: idValue,
            attributes: { name: 'Updated Name', website: 'https://updated.com' },
          });

          const result = await handleUpdateOperation(request, toolConfig, resourceType);
    ...
    testCases.forEach(({ resourceType, description, idParam, idValue }) => {
      describe(`for ${description}`, () => {
        const valueTestCases = [
          ...
          { type: 'array', value: ['item1', 'item2'], description: 'array value' },
          { type: 'object', value: { nested: 'object' }, description: 'object value' },
        ];
        ...
        valueTestCases.forEach(({ type, value, description: valueDesc }) => {
          it(`should successfully update attribute with ${valueDesc}`, async () => {
            ...
            await handleUpdateAttributeOperation(request, toolConfig, resourceType);

            expect(toolConfig.handler).toHaveBeenCalledWith(idValue, 'test_attribute', value);
            expect(mockFormatResponse).toHaveBeenCalledWith(
        ...
        it('should return error when attributeName parameter is missing', async () => {
          ...
          expect(mockCreateErrorResult).toHaveBeenCalledWith(
            expect.objectContaining({
              message: 'attributeName parameter is required for updateAttribute operation',
            }),
            `/${resourceType}/${idValue}`,
            'PATCH',
        ...
        it('should return error when value parameter is undefined', async () => {
          ...
          expect(mockCreateErrorResult).toHaveBeenCalledWith(
            expect.objectContaining({
              message: 'value parameter is required for updateAttribute operation',
            }),
            `/${resourceType}/${idValue}`,
            'PATCH',
        ...
        it('should allow null as a valid value', async () => {
          ...
          expect(toolConfig.handler).toHaveBeenCalledWith(idValue, 'test_attribute', null);
        });
    ...
    testCases.forEach(({ resourceType, description, idParam, idValue }) => {
      describe(`for ${description}`, () => {
        it('should successfully delete a record with valid id', async () => {
          ...
          const result = await handleDeleteOperation(request, toolConfig, resourceType);
        ...
        it('should use custom formatResult when provided', async () => {
          const mockResult = { deleted: true, id: idValue };
          const customFormatter = vi.fn().mockReturnValue('Custom delete result');
          const toolConfig = createMockToolConfig(mockResult, customFormatter);
          const request = createMockRequest('delete-company', {
            [idParam]: idValue,
          });
          ...
          expect(customFormatter).toHaveBeenCalledWith(mockResult);
          expect(mockFormatResponse).toHaveBeenCalledWith('Custom delete result');
        });
  ...
  describe('Edge cases and error scenarios', () => {
    it('should handle malformed request objects', async () => {
      ...
      await handleCreateOperation(malformedRequest, toolConfig, ResourceType.COMPANIES);
    ...
    it('should handle null arguments object', async () => {
      ...
      await handleCreateOperation(requestWithNullArgs, toolConfig, ResourceType.COMPANIES);
