  ...
  it('should have unique tool names across all config modules', () => {
    ...
    if (duplicates.size > 0) {
      console.error('Duplicate tool names found:', 
        Array.from(duplicates).map(name => 
          `${name} in: ${configSources[name]?.join(', ')}`
        )
      );
    }
    ...
    expect(Object.values(formatterConfigs).some(config => 
      config && typeof config === 'object' && config.name === basicInfoToolName
    )).toBe(true);
    
    expect(Object.values(crudToolConfigs).some(config => 
      config && typeof config === 'object' && config.name === basicInfoToolName
    )).toBe(false);
  });
});