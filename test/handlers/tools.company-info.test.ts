...
const mockedCompanyAttributes = companyAttributes as vi.Mocked<
  typeof companyAttributes
>;
...
describe('Company Info Tools', () => {
  beforeEach(() => {
    ...
    mockedRegistry.findToolConfig.mockImplementation((toolName: string) => {
      ...
      return {
        ...
        toolConfig: config,
        toolType:
          toolName === 'get-company-basic-info'
            ? 'basicInfo'
            : toolName === 'get-company-business-info'
              ? 'businessInfo'
              : toolName === 'get-company-fields'
                ? 'fields'
                : 'unknown',
      };
    });
  ...
  describe('Basic Info Tool', () => {
    it('should execute get-company-basic-info tool successfully', async () => {
      ...
      expect(mockedCompanyAttributes.getCompanyBasicInfo).toHaveBeenCalledWith(
        'company123'
      );
  ...
  describe('Business Info Tool', () => {
    it('should execute get-company-business-info tool successfully', async () => {
      ...
      expect(
        mockedCompanyAttributes.getCompanyBusinessInfo
      ).toHaveBeenCalledWith('company123');
      expect(result.isError).toBeFalsy();
  ...
  describe('Company Fields Tool', () => {
    it('should execute get-company-fields tool successfully', async () => {
      ...
      expect(mockedCompanyAttributes.getCompanyFields).toHaveBeenCalledWith(
        'company123',
        ['name', 'custom_field']
      );
      expect(result.isError).toBeFalsy();
  ...
  describe('Error Handling', () => {
    it('should handle API errors', async () => {
      // Arrange
      mockedCompanyAttributes.getCompanyBasicInfo.mockRejectedValue(
        new Error('API Error')
      );
