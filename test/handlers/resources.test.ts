...
describe('resources', () => {
  describe('registerResourceHandlers', () => {
    ...
    it('should handle list people request successfully', async () => {
      ...
      const result = await listResourcesHandler({
        params: { type: ResourceType.PEOPLE },
      });
    ...
    it('should handle read company resource request successfully', async () => {
      ...
      mockedParseResourceUri.mockReturnValue([
      ...
      const result = await readResourceHandler({
        params: { uri: 'attio://companies/company1' },
      });
