...
describe('field-mapper – uniqueness error enhancement', () => {
  ...
  it('returns original message when no attribute ID pattern found', async () => {
    const result = await enhanceUniquenessError(UniversalResourceType.COMPANIES, 'Uniqueness constraint violation on field "name"', { name: 'Duplicate Corp' });
    expect(result).toBeTypeOf('string');
  });

  it('returns original message when field cannot be extracted', async () => {
    const result = await enhanceUniquenessError(UniversalResourceType.COMPANIES, 'Generic uniqueness error', { name: 'Test Corp' });
    expect(result).toBeTypeOf('string');
  });

  it('attempts enhancement with attribute ID pattern', async () => {
    const result = await enhanceUniquenessError(UniversalResourceType.COMPANIES, 'Uniqueness constraint violation for attribute with ID "company-name"', { name: 'Test Corp' });
