...
import { detectFieldCollisions, mapRecordFields } from '../../../../src/handlers/tool-configs/universal/field-mapper.js';
...
describe('field-mapper – record mapping & collisions', () => {
  ...
  describe('detectFieldCollisions()', () => {
    it('detects no collisions for distinct targets', async () => {
      const result = await detectFieldCollisions(UniversalResourceType.COMPANIES, { name: 'Test', industry: 'Tech' });
    ...
    it('detects collisions when fields map to same target', async () => {
      const result = await detectFieldCollisions(UniversalResourceType.COMPANIES, { website: 'test.com', url: 'test.com' });
    ...
    it('ignores null-mapped fields', async () => {
      const result = await detectFieldCollisions(UniversalResourceType.COMPANIES, { name: 'Test' });
      expect(result.hasCollisions).toBe(false);
    });
  });
  ...
  describe('mapRecordFields()', () => {
    it('maps all fields from record data', async () => {
      const res = await mapRecordFields(UniversalResourceType.COMPANIES, { company_name: 'Test Corp', website: 'test.com' });
    ...
    it('detects and reports field collisions', async () => {
      const res = await mapRecordFields(UniversalResourceType.COMPANIES, { website: 'test.com', url: 'test.com' });
    ...
    it('preserves non-mapped fields', async () => {
      const res = await mapRecordFields(UniversalResourceType.COMPANIES, { name: 'Test Corp', custom_field: 'custom_value' });
      expect(res.mapped.name).toBe('Test Corp');
