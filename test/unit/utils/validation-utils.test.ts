...
describe('validation-utils', () => {
  ...
  describe('getResourceAttributes', () => {
    it('should fetch and cache resource attributes', async () => {
      const mockAttributes = [
        ...
        { api_slug: 'company_type', type: 'select', title: 'Type', options: [
  ...
  describe('validateSelectField', () => {
    ...
    it('should validate valid select option', async () => {
      const result = await validateSelectField('companies', 'company_type', 'startup');
    ...
    it('should reject invalid select option with helpful message', async () => {
      const result = await validateSelectField('companies', 'company_type', 'invalid_option');
      
      expect(result.isValid).toBe(false);
      expect(result.error).toContain("Invalid value 'invalid_option' for field 'company_type'");
      expect(result.error).toContain("Valid options are: ['Startup', 'Enterprise', 'Small Business', 'Non Profit']");
      expect(result.error).toContain('Please choose one of the valid values');
    });

    it('should pass validation for non-select fields', async () => {
      const result = await validateSelectField('companies', 'name', 'any_value');
      
      expect(result.isValid).toBe(true);
    });

    it('should handle missing field gracefully', async () => {
      const result = await validateSelectField('companies', 'non_existent_field', 'value');
  ...
  describe('validateMultiSelectField', () => {
    ...
    it('should validate valid multi-select options', async () => {
      const result = await validateMultiSelectField('companies', 'tags', ['important', 'urgent']);
    ...
    it('should reject invalid multi-select options', async () => {
      ...
      expect(result.error).toContain("Valid options are: ['Important', 'Urgent', 'Follow Up']");
    });
  });
  ...
  describe('validateReadOnlyFields', () => {
    ...
    it('should pass validation when no read-only fields are updated', async () => {
      const result = await validateReadOnlyFields('companies', { name: 'Test Company' });
    ...
    it('should reject updates to single read-only field', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toContain("Cannot update read-only field 'created_at'");
      expect(result.error).toContain('automatically managed by the system');
      expect(result.error).toContain('Remove this field from your update request');
    });
    ...
    it('should reject updates to multiple read-only fields', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toContain("Cannot update read-only fields 'created_at', 'updated_at'");
      expect(result.error).toContain('Remove these fields from your update request');
    });
  });
  ...
  describe('validateFieldExistence', () => {
    ...
    it('should validate existing fields', async () => {
      const result = await validateFieldExistence('companies', ['name', 'website']);
    ...
    it('should handle multiple field validation', async () => {
      const result = await validateFieldExistence('companies', ['name', 'invalid_field']);
  ...
  describe('validateRecordFields', () => {
    ...
    it('should validate valid record fields for creation', async () => {
      const result = await validateRecordFields('companies', {
        name: 'Test Company',
