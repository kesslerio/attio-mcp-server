...
describe('Tasks Field Mapping Fix - Issue #417', () => {
  ...
  it('should map common task field variations to correct field names', () => {
    ...
    expect(titleMapping.warnings.some(w => w.includes('assignee'))).toBe(true);
  ...
  it('should validate required fields for tasks', () => {
    ...
    const mappedResult = validateFields(UniversalResourceType.TASKS, invalidData);
  ...
  it('should provide helpful suggestions for unknown task fields', () => {
    ...
    const titleSuggestion = getFieldSuggestions(UniversalResourceType.TASKS, 'title');
    expect(titleSuggestion).toContain('content');
    
    const assigneeSuggestion = getFieldSuggestions(UniversalResourceType.TASKS, 'assignee');
    expect(assigneeSuggestion).toContain('assignee_id');
    
    const dueSuggestion = getFieldSuggestions(UniversalResourceType.TASKS, 'due');
    expect(dueSuggestion).toContain('due_date');
    
    const unknownSuggestion = getFieldSuggestions(UniversalResourceType.TASKS, 'completely_unknown_field');
