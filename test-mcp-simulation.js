...
async function simulateMCPRequest() {
  ...
  const { ResourceType, FilterConditionType } = await import('./dist/types/attio.js');
  ...
  const translated = translateAttributeNamesInFilters(claudeFilter, ResourceType.COMPANIES);
  ...
  const { companyToolConfigs } = await import('./dist/handlers/tool-configs/companies.js');
  ...
  if (advancedSearchConfig) {
    ...
    try {
      ...
      if (error.message?.includes('b2b_segment')) {
        console.error('ERROR: The handler is still seeing the untranslated attribute!');
      }
    }
  }
}
