...
| **get-record-details** | Retrieve specific record | `resource_type`, `record_id` | `fields` |
...
| **advanced-search** | Complex filtering | `resource_type` | `filters`, `sort_by`, `sort_order`, `limit` |

### Advanced Operations (P2 Priority)

| Tool Name | Purpose | Required Parameters | Optional Parameters |
|-----------|---------|-------------------|-------------------|
| **search-by-relationship** | Find related records | `relationship_type`, `source_id` | `target_resource_type`, `limit` |
| **search-by-content** | Content-based search | `resource_type`, `content_type`, `search_query` | `limit` |
| **search-by-timeframe** | Date range filtering | `resource_type`, `timeframe_type` | `start_date`, `end_date`, `limit` |
| **batch-operations** | Bulk operations | `resource_type`, `operation_type` | `record_ids`, `operations` |
| **batch-search** | Parallel searches | `resource_type`, `queries` | `limit` |
...
**Example Usage:**
```bash
mcp__attio__search-records resource_type="companies" query="technology" limit=10
```
...
**Example Usage:**
```bash
mcp__attio__get-record-details resource_type="people" record_id="abc123" fields='["name","email","title"]'
```
...
2. **Search:** Use `search-records` to find existing data
3. **Details:** Use `get-record-details` for complete record info
4. **Modify:** Use `create-record` or `update-record` as needed
5. **Cleanup:** Use `delete-record` to remove test data

### Advanced Workflow
1. **Complex Search:** Use `advanced-search` with filters
2. **Relationship Discovery:** Use `search-by-relationship`
3. **Content Search:** Use `search-by-content` for text queries
