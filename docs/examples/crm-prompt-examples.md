...
- `records.search_advanced` with `resource_type: "companies"`
...
- `records.search_by_timeframe` with `resource_type: "companies"`
- Date field: `created_at`, relative range: `last_30_days`
- `records.search` with query: "SaaS"
- `records.get_info` with `info_type: "business"` and `info_type: "social"`
...
- `records.search` with `resource_type: "companies"`, query: "fintech"
- `records.batch` with `operation_type: "update"`
...
- `records.search_advanced` with `resource_type: "people"`
- Filters: `job_title: { "$contains": ["VP", "Chief", "CTO", "CEO", "COO"] }`
- `records.search_by_relationship` to connect with company size data
...
- `records.search_by_relationship` with company employee filter
- `records.search_by_timeframe` to exclude recent contacts
...
- `records.search_by_relationship` with `resource_type: "people"`
- Related resource type: "companies" with list membership filter
- `records.get_info` with `info_type: "contact"`
...
- `records.search` with `resource_type: "companies"` and list filter
- `records.search_by_timeframe` with negative filter (no activity in timeframe)
...
- `records.search_by_relationship` to find primary contacts
- `records.search_by_content` to retrieve recent notes
...
- `records.search` with `resource_type: "people"` and list membership
- `records.search_by_relationship` to check company list memberships
...
- Complex filters:
  ...
  ```
- `records.search_by_relationship` to find Director+ contacts
- `records.search_by_timeframe` to exclude companies with recent activity
...
- `records.search_by_content` with content query: "renewal OR expansion"
...
- `records.search_by_content` to find support tickets with negative sentiment
...
- `records.search_by_content` to find notes mentioning expansion/upsell
...
- `records.search_by_content` with `resource_type: "companies"`
- Content types: ["notes"]
...
- `records.search_by_timeframe` with `resource_type: "people"`
- Date field: company change or job title update
- `records.search_by_relationship` to connect with target company lists
...
- `records.search` with `resource_type: "companies"`
...
- `records.search` to find companies in "Prospect" list
- `records.search_by_timeframe` for recent email engagement
- `records.batch` for bulk list membership updates
...
- `records.search` with `resource_type: "people"`
- Advanced filtering to identify duplicates by email/phone
- `records.batch` for record deduplication
...
Uses: `records.search` with `resource_type: "people"` + relationship filtering
...
Uses: `records.search_advanced` with multiple filters and relationship connections
...
Uses: `records.batch` for efficient bulk updates
...
1. `records.get_details` for Acme Corp
2. Extract industry, size, and other characteristics
3. `records.search_advanced` with similar company profile
...
1. **Start Simple**: Begin with `records.search` and `records.get_details`
2. **Add Complexity**: Layer in `records.search_advanced` and relationship queries
3. **Scale Up**: Use `records.batch` for multiple records
...
_These examples showcase the power and consistency of the universal tools system - one unified approach for all your CRM automation needs._
